...
renamed as (
    ...
    select
        ...
        CASE
            WHEN LENGTH(SPLIT_PART(param_vaca_partition_date, '::', 2)) = 1 THEN TO_DATE(CONCAT('1000-01-', SPLIT_PART(param_vaca_partition_date, '::', 2)), 'YYYY-MM-DD')
        ELSE TO_DATE(NULLIF(SPLIT_PART(param_vaca_partition_date, '::', 2), ''), 'YYYY-MM-DD')
        ...
        CASE
            WHEN LENGTH(SPLIT_PART(param_vacg_partition_date, '::', 2)) = 1 THEN TO_DATE(CONCAT('1000-01-', SPLIT_PART(param_vacg_partition_date, '::', 2)), 'YYYY-MM-DD')
        ELSE TO_DATE(NULLIF(SPLIT_PART(param_vacg_partition_date, '::', 2), ''), 'YYYY-MM-DD')
