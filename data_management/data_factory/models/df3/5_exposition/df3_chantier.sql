...
LEFT JOIN {{ source('python_load', 'metadata_zones') }} AS z ON t.zone_id = z.zone_id
--LEFT JOIN {{ source('python_load', 'metadata_porteurs') }} AS po 
...
LEFT JOIN {{ source('python_load', 'metadata_ppgs') }} AS ppg ON mc.ppg_id = ppg.ppg_id
LEFT JOIN {{ source('python_load', 'metadata_axes') }} AS ax ON ppg.ppg_axe = ax.axe_id
