...
(select * from {{ ref('agg_nat') }} where zone_id='FRANCE' and metric_type='vc'),
...
--	et de valeurs illegalement saisies
join_correct_and_illegal_values as (
select 
...
-- Jointure des VC avec le reste des valeurs NAT aggrégées
select 
...
from join_correct_and_illegal_values
union
(select * from {{ ref('agg_nat') }} where metric_type <>'vc')
