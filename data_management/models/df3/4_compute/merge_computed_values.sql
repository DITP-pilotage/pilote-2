...
vacp_v2.metric_date as date_valeur_proposition_v2,
...
left join {{ ref('compute_vacp_v2') }} vacp_v2 on a.indic_id = vacp_v2.indic_id and a.zone_id = vacp_v2.zone_id
...
left join (select * from {{ ref('get_vca_jalon') }} where jalon=(date_part('year', now()))) d2 on a.indic_id =d2.indic_id and a.zone_id =d2.zone_id
