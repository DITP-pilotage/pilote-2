...
mesure_last_params_nat_user AS (
    ...
    FROM mesure_last_params_nat AS a
    LEFT JOIN "qualif_249"."raw_data"."stg_ppg_metadata__zones" AS b ON a.zone_id = b.id
    WHERE
        (
            (metric_type = 'vi' AND vi_nat_from = 'user_input')
            OR (metric_type = 'va' AND va_nat_from = 'user_input')
            OR (metric_type = 'vc' AND vc_nat_from = 'user_input')
        )
        AND b.maille = 'NAT'
),
...
indic_agg_from_dept AS (
    ...
    FROM
        "qualif_249"."raw_data"."metadata_parametrage_indicateurs" AS b
    WHERE
        b.vi_nat_from NOT IN ('_', 'user_input')
        OR b.va_nat_from NOT IN ('_', 'user_input')
        OR b.vc_nat_from NOT IN ('_', 'user_input')
...
mesure_last_params_nat_from_dept AS (
    ...
    WHERE
        -- uniquement des données NAT avec parent NAT
        zone_type = 'DEPT'
        -- et pour lesquels l'agg NAT doit bien se faire depuis les données NAT
        AND (
            (metric_type = 'vi' AND vi_nat_from = 'DEPT')
            OR (metric_type = 'va' AND va_nat_from = 'DEPT')
            OR (metric_type = 'vc' AND vc_nat_from = 'DEPT')
        )
)
...
-- Ici on calcule une aggrégation en somme ET en moyenne des DEPT vers NAT
...
metric_type='vi'
and zone_type='DEPT' and zone_parent_type='NAT'
and vi_nat_from='DEPT'
