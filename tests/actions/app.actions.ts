...
export class AppActions {
  ...
  private async dismissPostLoginModals(): Promise<void> {
    const videoModal = this.page.getByRole("dialog").getByRole("button", { name: /vidéo/ });
    const newsletterModal = this.page
      .getByRole("dialog")
      .getByRole("button", { name: /continuer/i });

    if (await videoModal.isVisible({ timeout: 2000 }).catch(() => false)) {
      await videoModal.click();
    }
    if (await newsletterModal.isVisible({ timeout: 2000 }).catch(() => false)) {
