...
test('doit pouvoir exporter les données des indicateurs sous format CSV', async ({ page }) => {
  ...
  await test.step("Retour à l'étape 3 - Suppression des champs (Pour pouvoir tester le fichier après sinon trop de temps à DL)", async () => {
    ...
    await page.getByRole('checkbox', { name: /données de l'indicateur sur le territoire/ }).setChecked(false, { force: true });
    await page.getByRole('checkbox', { name: /données du chantier associé sur le territoire/ }).setChecked(false, { force: true });
