...
test('doit pouvoir exporter les données des indicateurs sous format CSV', async ({ page }) => {
  ...
  await test.step("Choix de l'export indicateurs", async () => {
    ...
    await page.waitForURL('**/accueil/chantier/NAT-FR?isModaleExportCsvOuverte=true&etapeCourante=1&typeExport=indicateurs');
  });

  await test.step("Passage à l'étape 2 - Périmètre de l'export", async () => {
    await page.getByRole('button', { name: /Étape suivante/ }).click();
    await page.waitForURL('**/accueil/chantier/NAT-FR?isModaleExportCsvOuverte=true&etapeCourante=2&typeExport=indicateurs');
  ...
  await test.step("Passage à l'étape 3 - Données à collecter", async () => {
    await page.getByRole('button', { name: /Étape suivante/ }).click();
    await page.waitForURL('**/accueil/chantier/NAT-FR?isModaleExportCsvOuverte=true&etapeCourante=3&typeExport=indicateurs');
  ...
  await test.step("Vérification des possibilités de choix d'export - cadrage de la PPG", async () => {
  ...
  await test.step("Vérification des possibilités de choix d'export - responsabilités de l’indicateur et de la PPG associée", async () => {
    await expect(page.getByLabel(/responsabilités de l’indicateur et de la PPG associée/)).toBeVisible();
    await page.getByRole('checkbox', { name: /responsabilités de l’indicateur et de la PPG associée/ }).setChecked(true, { force: true });
    await page.waitForURL('**/accueil/chantier/NAT-FR?isModaleExportCsvOuverte=true&etapeCourante=3&typeExport=indicateurs&optionsExport=identifiant,cadrage,gouvernance,responsabilite');
    await expect(page.getByLabel(/responsabilités de l’indicateur et de la PPG associée/)).toBeChecked();
  });

  await test.step("Vérification des possibilités de choix d'export - objectifs de la PPG associée", async () => {
    await expect(page.getByLabel(/objectifs de la PPG associée/)).toBeVisible();
    await page.getByRole('checkbox', { name: /objectifs de la PPG associée/ }).setChecked(true, { force: true });
    await page.waitForURL('**/accueil/chantier/NAT-FR?isModaleExportCsvOuverte=true&etapeCourante=3&typeExport=indicateurs&optionsExport=identifiant,cadrage,gouvernance,responsabilite,objectif');
    await expect(page.getByLabel(/objectifs de la PPG associée/)).toBeChecked();
  });
  ...
  await test.step("Vérification des possibilités de choix d'export - données descriptives de l’indicateur et de la PPG associée sur le territoire", async () => {
    ...
    await page.waitForURL('**/accueil/chantier/NAT-FR?isModaleExportCsvOuverte=true&etapeCourante=3&typeExport=indicateurs&optionsExport=identifiant,cadrage,gouvernance,responsabilite,objectif,description');
    await expect(page.getByLabel(/données descriptives de l’indicateur et de la PPG associée sur le territoire/)).toBeChecked();
  });

  await test.step("Vérification des possibilités de choix d'export - données de comparaison de l’indicateur", async () => {
    await expect(page.getByLabel(/données de comparaison de l’indicateur/)).toBeVisible();
    await page.getByRole('checkbox', { name: /données de comparaison de l’indicateur/ }).setChecked(true, { force: true });
    await page.waitForURL('**/accueil/chantier/NAT-FR?isModaleExportCsvOuverte=true&etapeCourante=3&typeExport=indicateurs&optionsExport=identifiant,cadrage,gouvernance,responsabilite,objectif,description,comparaison');
    await expect(page.getByLabel(/données de comparaison de l’indicateur/)).toBeChecked();
  });
  ...
  await test.step("Vérification des possibilités de choix d'export - météo et synthèse des résultats de la PPG associée sur le territoire", async () => {
    ...
    await page.waitForURL('**/accueil/chantier/NAT-FR?isModaleExportCsvOuverte=true&etapeCourante=3&typeExport=indicateurs&optionsExport=identifiant,cadrage,gouvernance,responsabilite,objectif,description,comparaison,synthese');
    await expect(page.getByLabel(/météo et synthèse des résultats de la PPG associée sur le territoire/)).toBeChecked();
  });

  await test.step("Vérification des possibilités de choix d'export - commentaires de la PPG associée", async () => {
    await expect(page.getByLabel(/commentaires de la PPG associée/)).toBeVisible();
    await page.getByRole('checkbox', { name: /commentaires de la PPG associée/ }).setChecked(true, { force: true });
    await page.waitForURL('**/accueil/chantier/NAT-FR?isModaleExportCsvOuverte=true&etapeCourante=3&typeExport=indicateurs&optionsExport=identifiant,cadrage,gouvernance,responsabilite,objectif,description,comparaison,synthese,commentaire');
    await expect(page.getByLabel(/commentaires de la PPG associée/)).toBeChecked();
  });

  await test.step("Vérification des possibilités de choix d'export - suivi des décisions stratégiques de la PPG associée", async () => {
    await expect(page.getByLabel(/suivi des décisions stratégiques de la PPG associée/)).toBeVisible();
    await page.getByRole('checkbox', { name: /suivi des décisions stratégiques de la PPG associée/ }).setChecked(true, { force: true });
    await page.waitForURL('**/accueil/chantier/NAT-FR?isModaleExportCsvOuverte=true&etapeCourante=3&typeExport=indicateurs&optionsExport=identifiant,cadrage,gouvernance,responsabilite,objectif,description,comparaison,synthese,commentaire,decision');
    await expect(page.getByLabel(/suivi des décisions stratégiques de la PPG associée/)).toBeChecked();
  });

  await test.step("Passage à l'étape 4 - Récapitulatif et validation - partie vérification transmission paramètre", async () => {
    await page.getByRole('button', { name: /Étape suivante/ }).click();
    await page.waitForURL('**/accueil/chantier/NAT-FR?isModaleExportCsvOuverte=true&etapeCourante=4&typeExport=indicateurs&optionsExport=identifiant,cadrage,gouvernance,responsabilite,objectif,description,comparaison,synthese,commentaire,decision');
  ...
  await test.step("Retour à l'étape 3 - Suppression des champs (Pour pouvoir tester le fichier après sinon trop de temps à DL)", async () => {
    ...
    await page.getByRole('checkbox', { name: /cadrage de l'indicateur/ }).setChecked(false, { force: true });
    await page.getByRole('checkbox', { name: /responsabilités de l’indicateur et de la PPG associée/ }).setChecked(false, { force: true });
    await page.getByRole('checkbox', { name: /objectifs de la PPG associée/ }).setChecked(false, { force: true });
    ...
    await page.getByRole('checkbox', { name: /données de comparaison de l’indicateur/ }).setChecked(false, { force: true });
    ...
    await page.getByRole('checkbox', { name: /commentaires de la PPG associée/ }).setChecked(false, { force: true });
    await page.getByRole('checkbox', { name: /suivi des décisions stratégiques de la PPG associée/ }).setChecked(false, { force: true });
  ...
  await test.step("Étape 4 - Récapitulatif et validation - partie téléchargement et vérification du fichier avec d'autres colonnes", async () => {
    ...
    await test.step('vérification du fichier identifiant, cadrage, responsabilite et objectif', async () => {
      const contents = await fs.promises.readFile(await download.path());
