...
test.describe("Formulaire indicateur — Consultation et modification", () => {
  test("doit afficher la fiche indicateur en mode consultation puis permettre la modification", async ({
    ...
    await test.step("Vérification du lien retour", async () => {
      await pageForm.expectLienRetourVisible();
    });

    await test.step("Vérification du sélecteur Actif/Inactif", async () => {
      await pageForm.expectSelecteurActifInactifVisible();
    });

    await test.step("Vérification du tableau récapitulatif", async () => {
      await pageForm.expectTableauRecapitulatifVisible();
      await pageForm.expectColonneRecapitulatif("Chantier associé");
      await pageForm.expectColonneRecapitulatif("Nom du chantier");
      await pageForm.expectColonneRecapitulatif("Identifiant indicateur");
      await pageForm.expectColonneRecapitulatif("Nom de l'indicateur");
      await pageForm.expectColonneRecapitulatif("Création de l'indicateur");
      await pageForm.expectColonneRecapitulatif("Dernière modification");
    });

    await test.step("Vérification de la structure des accordéons", async () => {
    ...
    await test.step("Vérification du mode consultation (lecture seule)", async () => {
      await pageForm.expectModeConsultation();
    });

    await test.step("Vérification des données de l'indicateur IND-021 dans la section identité", async () => {
      await pageForm.expectDonneeAffichee("CH-129");
      await pageForm.expectDonneeAffichee("IND-021");
    });

    await test.step("Ouverture de l'accordéon Paramétrages et vérification des sections", async () => {
    ...
    await test.step("Ouverture de l'accordéon Autres informations", async () => {
      await pageForm.ouvrirAccordionAutresInformations();
    });

    await test.step("Passage en mode modification", async () => {
      await pageForm.clickModifier();
      await pageForm.expectModeModification();
    });

    await test.step("Annulation des modifications — retour au mode consultation", async () => {
      await pageForm.clickAnnuler();
      await pageForm.expectModeConsultation();
    });

    await test.step("Modification de l'indicateur et soumission", async () => {
      await pageForm.clickModifier();
      await pageForm.expectModeModification();
    ...
    await test.step("Retour au listing depuis la fiche", async () => {
...
test.describe("Formulaire indicateur — Règles d'activation/désactivation des paramétrages", () => {
  test("doit activer/désactiver les champs d'agrégation selon l'origine des valeurs dans les mailles", async ({
    ...
    await test.step("Maille départementale — vi_dept_op désactivé quand vi_dept_from=user_input", async () => {
      await pageForm.expectSelecteurValeur("viDeptFrom", "user_input");
      await pageForm.expectSelecteurDesactive("viDeptOp");
    });

    await test.step("Maille départementale — changer vi_dept_from en sub_indic active vi_dept_op", async () => {
      await pageForm.changerSelecteur("viDeptFrom", "sub_indic");
      await pageForm.expectSelecteurActive("viDeptOp");
    });

    await test.step("Maille départementale — remettre vi_dept_from en user_input désactive vi_dept_op", async () => {
      await pageForm.changerSelecteur("viDeptFrom", "user_input");
      await pageForm.expectSelecteurDesactive("viDeptOp");
    });

    // IND-021 seed: va_dept_from=user_input → va_dept_op désactivé
    await test.step("Maille départementale — va_dept_op désactivé quand va_dept_from=user_input", async () => {
    ...
    await test.step("Maille régionale — vi_reg_op activé quand vi_reg_from=DEPT", async () => {
    ...
    await test.step("Maille régionale — changer vi_reg_from en user_input désactive vi_reg_op", async () => {
      await pageForm.changerSelecteur("viRegFrom", "user_input");
      await pageForm.expectSelecteurDesactive("viRegOp");
    });

    await test.step("Maille régionale — remettre vi_reg_from en DEPT active vi_reg_op", async () => {
      await pageForm.changerSelecteur("viRegFrom", "DEPT");
      await pageForm.expectSelecteurActive("viRegOp");
    });

    // IND-021 seed: vi_nat_from=DEPT → vi_nat_op=sum (activé)
    await test.step("Maille nationale — vi_nat_op activé quand vi_nat_from=DEPT", async () => {
      await pageForm.expectSelecteurValeur("viNatFrom", "DEPT");
      await pageForm.expectSelecteurActive("viNatOp");
    });

    await test.step("Maille nationale — changer vi_nat_from en _ désactive vi_nat_op", async () => {
    ...
    await test.step("Annulation des modifications", async () => {
      await pageForm.clickAnnuler();
      await pageForm.expectModeConsultation();
    });
  });
  ...
  test("doit gérer les règles du calcul de la valeur d'avancement", async ({
    ...
    await test.step("Opération TA annuel et TA global sont toujours désactivés", async () => {
    ...
    await test.step("Valeurs initiales du calcul — partition_date=from_year_start et op=sum", async () => {
    ...
    await test.step("Changer partition_date TA annuel en _ → op passe à current_value et vacg se synchronise", async () => {
    ...
    await test.step("Changer partition_date TA global en from_year_start → op revient à sum et vaca se synchronise", async () => {
    ...
    await test.step("Synchronisation decumul_from — vaca et vacg ont la même valeur", async () => {
      const valeurVaca = await page
        .locator('select[name="paramVacaDecumulFrom"]')
        .inputValue();
      await pageForm.expectSelecteurValeur("paramVacgDecumulFrom", valeurVaca);
    });
  ...
  test("doit désactiver les pondérations dept/reg quand l'indicateur n'est pas territorialisé", async ({
    ...
    await test.step("Pondération dept et reg activées quand indicateur territorialisé", async () => {
    ...
    await test.step("Désactiver la territorialisation désactive les pondérations dept et reg", async () => {
...
test.describe("Formulaire indicateur — Création", () => {
  test("doit afficher le formulaire en mode création et permettre de créer un indicateur", async ({
    ...
    await test.step("Connexion et accès au formulaire de création", async () => {
      await appActions.loginAs(DITP_ADMIN);
      await pageForm.gotoCreation(nouvelIndicateurId);
      await pageForm.expectTitreFiche(nouvelIndicateurId);
    });

    await test.step("Vérification du mode création", async () => {
      await pageForm.expectModeCreation();
    });

    await test.step("Vérification de la structure du formulaire en création", async () => {
      await pageForm.expectSelecteurActifInactifVisible();
      await pageForm.expectAccordionIdentiteOuvert();
      await pageForm.expectAccordionParametragesFerme();
      await pageForm.expectAccordionAutresInformationsFerme();
...
test.describe("Formulaire indicateur — Navigation depuis le listing", () => {
  test("doit naviguer du listing vers la fiche et revenir", async ({
    ...
    await test.step("Navigation vers la fiche IND-021 depuis le listing", async () => {
      await pageIndicateurs.rechercherIndicateur("IND-021");
      await pageIndicateurs.clickIndicateurParId("IND-021");
      await pageForm.expectTitreFiche("IND-021");
    });

    await test.step("Retour au listing via le lien Retour", async () => {
      await pageForm.clickRetour();
...
test.describe("Formulaire indicateur — Accès refusé", () => {
  test("doit refuser l'accès à la fiche indicateur aux profils non DITP_ADMIN", async ({
    ...
    await test.step("Équipe direction de projet — accès direct par URL refusé", async () => {
