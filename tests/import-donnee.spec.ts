...
test('doit pouvoir importer des données', async ({ page }) => {
  ...
  await test.step("Vérification que le fichier n'est pas conforme", async () => {
    await expect(page.getByText(/Bravo, le fichier est conforme !/)).not.toBeVisible();
    await expect(page.getByText(/Le fichier ne peut pas être importé/)).toBeVisible();
    await expect(page.getByText(/IND-97 ne respecte pas le motif imposé/)).toBeVisible();
    await expect(page.getByText(/L'indicateur IND-97 ne correpond pas à l'indicateur choisis \(IND-927\)/)).toBeVisible({
      timeout: 30_000,
    });
  });
