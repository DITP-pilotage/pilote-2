...
**Objectif** : Désactiver effectivement les comptes inactifs depuis plus de 100 jours

#### Tâche 4.1 : Ajouter une fonction pour récupérer l'ID utilisateur depuis l'email

**Fichier** : `src/server/gestion-utilisateur/infrastructure/adapters/PrismaUtilisateurRepository.ts`

**Action** : Créer une nouvelle méthode `recupererUtilisateurId(email: string): Promise<string | null>`

Cette fonction permettra de récupérer l'ID de l'utilisateur système qui sera l'auteur des désactivations automatiques.
...
#### Tâche 4.2 : Mettre à jour l'interface UtilisateurRepository

**Fichier** : `src/server/gestion-utilisateur/domain/ports/UtilisateurRepository.ts`

**Action** : Ajouter la signature de la méthode dans l'interface :

```typescript
recupererUtilisateurId(email: string): Promise<string | null>;
```

#### Tâche 4.3 : Implémenter la désactivation dans le script

**Fichier** : `scripts/desactivationComptesInactifs.ts`

**Inspiration** : `src/server/gestion-utilisateur/usecases/DesactiverUnUtilisateurUseCase.ts:91-101`

**Étapes détaillées** :

1. **Récupérer l'ID de l'utilisateur système**
   - Définir un email système (ex: `systeme@interieur.gouv.fr`)
   - Utiliser `utilisateurRepository.recupererUtilisateurId()` pour récupérer son ID
   - Si l'utilisateur n'existe pas, arrêter le script avec une erreur explicite

2. **Résoudre les repositories nécessaires**
...
3. **Pour chaque compte à désactiver (> 100 jours d'inactivité)**, effectuer les opérations suivantes dans l'ordre :

   a. **Désactivation dans la base de données**

   ```typescript
   await utilisateurRepository.desactiver(email, auteurIdSysteme);
   ```

   - Référence : `PrismaUtilisateurRepository.ts:268-279`
   - Met à jour `date_desactivation`, `date_modification` et `auteur_id_modification`

   b. **Suppression du contact Brevo** (si feature flag actif)
   ...
   c. **Désactivation dans Keycloak** (si configuré)
   ...
   - Référence : `UtilisateurIAMKeycloakRepository.ts:34-49`
   - Met `enabled: false` sur le compte Keycloak

   d. **Suppression du token API**

   ```typescript
   await tokenAPIInformationRepository.supprimerTokenAPIInformation({ email });
   ```

---

## Notes Techniques pour l'Étape 4

### Email de l'utilisateur système

À déterminer selon l'environnement :

- Production : Utiliser un email système existant
- Développement : Créer ou utiliser un compte de test

### Variables d'environnement utilisées

- `NEXT_PUBLIC_FF_LIEN_CONTACT_BREVO` : Active/désactive la suppression des contacts Brevo
- `IMPORT_KEYCLOAK_URL` : Détermine si la désactivation Keycloak doit être effectuée

### Ordre des opérations

L'ordre des opérations de désactivation est important :

1. Base de données (PILOTE)
2. Brevo (infolettre)
3. Keycloak (authentification)
4. Token API (accès API)

Cet ordre garantit que l'utilisateur est d'abord marqué comme désactivé dans le système principal.
