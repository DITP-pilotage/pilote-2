...
WHERE 
  ...
  indic_id IN (SELECT indic_id FROM {{ source('ppg_metadata', 'metadata_indicateurs_hidden') }} WHERE indic_hidden_pilote = TRUE) OR
  -- OU si l'indic_id n'est pas dans metadata_indicateurs_hidden
  indic_id NOT IN (SELECT indic_id FROM {{ source('ppg_metadata', 'metadata_indicateurs_hidden') }})
