...
psql -q -d "$DATABASE_URL" -c "\COPY raw_data.metadata_porteurs (porteur_id, porteur_short, porteur_name, porteur_desc, porteur_type, porteur_directeur, porteur_picto) FROM '$SCRIPT_DIR/$RAW_DATA_SEED_DIR/metadata_porteurs.csv' WITH (FORMAT csv, HEADER true)"
