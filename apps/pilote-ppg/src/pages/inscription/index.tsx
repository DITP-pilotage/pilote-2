...
export async function getServerSideProps(
  ...
  const contactAjouterALaListe = await getContainer("gestionUtilisateur")
    .resolve("ajouterUnContactAUneInfoLettreUseCase")
    .execute(session.user.id, 11);
  ...
  return (
    <>
      <Head>
        <title>
          {contactAjouterALaListe
            ? "Minute Pilote - Inscription validée"
            : "Minute Pilote - Erreur inscription"}
        </title>
      </Head>
      <main>
        {contactAjouterALaListe ? (
