...
describe("ModifierCommentairePublieUseCase", () => {
  let useCase: ModifierCommentairePublieUseCase;
  let commentaireRepository: MockProxy<CommentaireRepository>;

  beforeEach(() => {
    commentaireRepository = mock<CommentaireRepository>();
    useCase = new ModifierCommentairePublieUseCase({ commentaireRepository });
  });

  it("modifie un commentaire PUBLIE et le sauvegarde en PUBLIE", async () => {
    // given
  ...
  it("rejette un commentaire BROUILLON sans appeler save", async () => {
    // given - un brouillon passé par erreur comme ID de commentaire publié
    commentaireRepository.getById.mockResolvedValue(
