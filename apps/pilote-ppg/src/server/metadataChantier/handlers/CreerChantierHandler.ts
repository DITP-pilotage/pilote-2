...
export const creerChantierCommandSchema = z.object({
...
export type CreerChantierCommand = z.infer<typeof creerChantierCommandSchema>;

export class CreerChantierHandler {
  ...
  async execute(command: CreerChantierCommand): Promise<void> {
    await this.prisma.getInstance().metadata_chantiers.create({
      data: {
        chantier_id: command.chantierId,
        ch_nom: command.chNom,
        ch_descr: command.chDescr,
        ch_ppg: command.chPpg,
        ch_territo: command.chTerrito,
        ch_hidden_pilote: command.chHiddenPilote,
