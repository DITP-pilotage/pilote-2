...
describe("ImportDonneeIndicateurAPIHandler", () => {
  describe("quand on import un fichier via une donnee en JSON", () => {
    it(
      "quand le fichier est invalide a cause de validata, doit remonter les erreurs validata",
    ...
    it(
      "quand le fichier est invalide a cause de validata, doit sauvegarder les erreurs validata",
      createIntegrationTest(async () => {
        // Given
        const auteurId = await creeUnUtilisateurEnBase();
        const report = new ReportValidataWithDataBuilder()
    ...
    it(
      "quand le fichier est invalide a cause de règle DITP <<indic_id request different des indic_id dans fichier>>, doit sauvegarder les erreurs",
      createIntegrationTest(async () => {
        // Given
        const auteurId = await creeUnUtilisateurEnBase();
        const report = new ReportValidataWithDataBuilder()
        ...
        expect(listeErreursValidationFichier[2].nom_du_champ).toEqual(
          "indic_id",
        );
      }),
    );
    ...
    it(
      "quand le fichier est valide, doit importer les données",
      createIntegrationTest(async () => {
        // Given
        const auteurId = await creeUnUtilisateurEnBase();
