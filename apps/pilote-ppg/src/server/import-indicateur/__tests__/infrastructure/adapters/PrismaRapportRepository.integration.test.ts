...
describe("PrismaRapportRepository", () => {
  ...
  beforeEach(() => {
    ...
    prismaMesureIndicateurTemporaireRepository =
      new PrismaMesureIndicateurTemporaireRepository();
  });
  describe("#sauvegarder", () => {
    it(
      "doit sauvegarder le rapport",
      createIntegrationTest(async () => {
        ...
        expect(new Date(listeDesRapports[0].date_creation).getTime()).toEqual(
          now.getTime(),
        );
        expect(listeDesRapports[0].utilisateurEmail).toEqual(
  ...
  describe("#récupérerRapportParId", () => {
    it(
      "doit récupérer le rapport avec les données",
      createIntegrationTest(async () => {
        // Given
        const auteurId = await creeUnUtilisateurEnBase();
        const now = new Date();
        ...
        // When
        const rapportResult =
          await prismaRapportRepository.récupérerRapportParId(
            "f69bbd1f-de95-442a-9392-df644e1096f8",
          );
        ...
        expect(rapportResult.listeMesuresIndicateurTemporaire[0].id).toEqual(
          "b2450ce3-8006-4550-8132-e5aab19c0caf",
        );
        expect(
          rapportResult.listeMesuresIndicateurTemporaire[0].indicId,
        ).toEqual("IND-001");
        expect(
          rapportResult.listeMesuresIndicateurTemporaire[0].metricDate,
        ).toEqual("30/12/2022");
        expect(
          rapportResult.listeMesuresIndicateurTemporaire[0].metricType,
        ).toEqual("vi");
        expect(
          rapportResult.listeMesuresIndicateurTemporaire[0].metricValue,
        ).toEqual("12");
        expect(
          rapportResult.listeMesuresIndicateurTemporaire[0].zoneId,
        ).toEqual("D001");

        expect(rapportResult.listeMesuresIndicateurTemporaire[1].id).toEqual(
          "f7632d30-5b49-465e-8774-063f9f67f83b",
        );
        expect(
          rapportResult.listeMesuresIndicateurTemporaire[1].indicId,
        ).toEqual("IND-002");
        expect(
          rapportResult.listeMesuresIndicateurTemporaire[1].metricDate,
        ).toEqual("31/12/2022");
        expect(
          rapportResult.listeMesuresIndicateurTemporaire[1].metricType,
        ).toEqual("vc");
        expect(
          rapportResult.listeMesuresIndicateurTemporaire[1].metricValue,
        ).toEqual("15");
        expect(
          rapportResult.listeMesuresIndicateurTemporaire[1].zoneId,
        ).toEqual("D002");
      }),
    );
  });
