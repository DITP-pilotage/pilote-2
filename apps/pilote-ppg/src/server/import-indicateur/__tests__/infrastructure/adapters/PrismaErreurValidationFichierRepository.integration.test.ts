...
describe("PrismaErreurValidationFichierRepository", () => {
  ...
  beforeEach(() => {
    ...
    prismaErreurValidationFichierRepository =
      new PrismaErreurValidationFichierRepository();
  });
  ...
  describe("#sauvegarder", () => {
    it(
      ...
      createIntegrationTest(async () => {
        ...
        expect(resultListeErreursValidationFichier[0].rapport_id).toEqual(
          "a0c086eb-21e2-4f00-9ca8-4b0fcce133ad",
        );
        expect(resultListeErreursValidationFichier[0].nom).toEqual(
          "Cellule vide",
        );
        ...
        expect(resultListeErreursValidationFichier[0].numero_de_ligne).toEqual(
          1,
        );
        expect(
          resultListeErreursValidationFichier[0].position_de_ligne,
        ).toEqual(1);
        expect(
          resultListeErreursValidationFichier[0].position_du_champ,
        ).toEqual(1);

        expect(resultListeErreursValidationFichier[1].id).toBeDefined();
        expect(resultListeErreursValidationFichier[1].rapport_id).toEqual(
          "a0c086eb-21e2-4f00-9ca8-4b0fcce133ad",
        );
        expect(resultListeErreursValidationFichier[1].nom).toEqual(
          "METRIC_INVALIDE",
        );
        expect(resultListeErreursValidationFichier[1].cellule).toEqual(
          "IND-02",
        );
        expect(resultListeErreursValidationFichier[1].message).toEqual(
          "Indicateur invalide",
        );
        expect(resultListeErreursValidationFichier[1].nom_du_champ).toEqual(
          "indic_id",
        );
        expect(resultListeErreursValidationFichier[1].numero_de_ligne).toEqual(
          2,
        );
        expect(
          resultListeErreursValidationFichier[1].position_de_ligne,
        ).toEqual(2);
        expect(
          resultListeErreursValidationFichier[1].position_du_champ,
        ).toEqual(2);
      }),
    );
  });
