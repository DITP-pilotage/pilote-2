...
import { searchIndicateursInfiniteQueryOptions } from '@/queries/permissions'

export type IndicateurHit = { publicId: string; nom: string }

export function IndicateurSearchModal({
  excludedPublicIds,
  onSelect,
  onClose,
}: {
  excludedPublicIds: string[]
  onSelect: (hit: IndicateurHit) => void
  onClose: () => void
}) {
  ...
  const hits: IndicateurHit[] = (query.data?.pages ?? [])
    ...
    >
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-text">Ajouter un indicateur</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Fermer"
        ...
        <div className="mb-4 space-y-3">
          <div>
            <label
              htmlFor="indicateur-search-identifiant"
              className="mb-1.5 block text-xs font-semibold"
            >
              Identifiant
            </label>
            <input
              id="indicateur-search-identifiant"
              autoFocus
          ...
          <div>
            <label htmlFor="indicateur-search-nom" className="mb-1.5 block text-xs font-semibold">
              Nom
            </label>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-text-subtle" />
              <input
                id="indicateur-search-nom"
