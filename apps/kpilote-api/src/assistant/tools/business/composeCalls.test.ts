...
describe('composeCalls', () => {
  it('assemble les réponses sous les clés demandées', async () => {
    ...
    const resultat = await composeCalls(requeteur, { a: '/un', b: '/deux' })
  ...
  it('lance les appels en parallèle', async () => {
    ...
    const promesse = composeCalls(requeteur, { a: '/un', b: '/deux' })
  ...
  it('porte la reason d’indisponibilité plutôt qu’un null nu', async () => {
    const requeteur = vi.fn(() => Promise.resolve(new Response('non', { status: 403 })))
    const resultat = await composeCalls(requeteur, { a: '/un' })

    expect(resultat.a).toEqual({ unavailable: expect.stringContaining('403') })
  })
  ...
  it('distingue un refus de droit d’une absence de données', async () => {
    ...
    const resultat = await composeCalls(requeteur, { vide: '/ok', refuse: '/interdit' })

    expect(resultat.vide).toEqual({ data: { items: [] } })
    expect('unavailable' in (resultat.refuse as object)).toBe(true)
  })
})
...
describe('get_synthese_indicateur', () => {
  ...
  it('paramètre les valeurs remarquables avec les référentiels lus dans l’identité', async () => {
    ...
    const outil = createGetSyntheseIndicateurTool(requeteur)
  ...
  it('n’appelle pas les routes qui exigent un territoire quand aucun n’est fourni', async () => {
    ...
    const sortie = (await outil.execute?.({ id: 'IND-42' }, { toolCallId: 't', messages: [] })) as {
      tauxProgression: { unavailable?: string }
    }
    ...
    expect(sortie.tauxProgression.unavailable).toContain('individuId')
  })
