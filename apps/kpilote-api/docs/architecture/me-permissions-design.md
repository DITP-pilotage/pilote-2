...
principal courant a le droit d'écrire sur une ressource (panier, indicateur)
...
Premier cas remonté : onglet « Commentaires » du panier (PIL-1588). Le besoin
...
on s'appuie sur `paniers` / `indicateurs`. Cela évite d'aligner le client sur
...
Règles confirmées par lecture du code (`apps/mb-api/src/panier/permissions.ts`
et `apps/mb-api/src/indicateur/permissions.ts`) :

| Action | Source |
|---|---|
| READ panier | direct uniquement (la visibilité PUBLIC n'est pas matérialisée) |
| WRITE panier | direct uniquement |
| READ indicateur | direct **OU** propagé depuis un panier où le principal a READ ou WRITE |
| WRITE indicateur | direct uniquement (**pas de propagation**) |

Concrètement, on assemble la réponse à partir de :

1. **`PanierPermission`** du principal → liste `paniers[]`.
2. **`IndicateurPermission`** du principal → contributions `READ` et/ou
   `WRITE` à `indicateurs[]`.
3. **Propagation panier → indicateur** : pour chaque panier de (1) (qu'il
   porte READ ou WRITE), on ajoute `READ` sur chacun des indicateurs liés
   via `PanierIndicateur`. **WRITE ne propage jamais**.
...
export const listerMesPermissions = (): ResultAsync<MePermissionsApiModel, never> => {
  ...
  ])).andThen(([panierPerms, indicateurPerms]) =>
    // joindre PanierIndicateur pour propagation READ, dédupliquer, trier
    ...
  )
}
```
...
| # | Question | Décision |
|---|---|---|
| 1 | Forme de la réponse pour un admin | `{ isAdmin: true, paniers: [], indicateurs: [] }`. Le champ `isAdmin` est **optionnel** et n'est présent que lorsqu'il vaut `true` — pas de `isAdmin: false` pour les principals standards. |
| 2 | Propagation `WRITE` panier → indicateur | **Non**. Confirmé par lecture de `ensureIndicateurWritePermission` : le WRITE indicateur est strictement direct. La propagation panier → indicateur ne concerne que `READ`. |
| 3 | Format d'`id` | **`publicId`** (`PAN-…`, `IND-…`). On ne renvoie jamais d'UUID interne — la webapp ne manipule que des publicIds. |
...
- Hook `useCanWritePanier(panierId)` / `useCanWriteIndicateur(indicateurId)`.
- Câblage dans `PanierCommentaireConfigProvider` et
...
- Intégration `listerMesPermissions` :
  - principal sans permission → `{ paniers: [], indicateurs: [] }`
  - principal avec WRITE sur PAN-005 → entrée PAN-005 + IND-001..003
    propagés en READ.
  - dédup : permission directe READ sur IND-001 + READ propagé via PAN-005 →
