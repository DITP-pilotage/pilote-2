...
const AAccesATousLesUtilisateurs = (profil: Profil | null) => {
  if (profil)
...
export default function useSectionDétailsMetadataAutresIndicateurForm() {
  ...
  const ChangementProfilSelectionne = (profilSélectionne: ProfilCode) => {
    setValue('profil', profilSélectionne);
    ...
    setValue('gestionUtilisateur', false);
    setValue('saisieIndicateur', false);
    setValue('saisieCommentaire', false);
  };
