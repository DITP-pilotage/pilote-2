...
const presenterEnFonctionDuSchema = (
  informationsIndicateur: InformationIndicateurContrat[],
  valeurSelectionné: string,
) => {
  const indicateurSelectionné = informationsIndicateur.find(
    (indicateur) => indicateur.indicId === valeurSelectionné,
  );
    ...
    <div>
      {options.length > 0 ? (
        ...
        <div>
          {
            wording.PAGE_IMPORT_MESURE_INDICATEUR.SECTION_ETAPE_IMPORT
              .ETAPE_SELECTION_INDICATEUR.MESSAGE_ERREUR_AUCUN_INDICATEUR
          }
        </div>
      )}
    </div>
