...
} from "./PublicationFormulaire.interface";

export default function usePublicationFormulaire(
  succèsCallback: PublicationFormulaireProps["succèsCallback"],
  erreurCallback: PublicationFormulaireProps["erreurCallback"],
) {
  const mutationCréerPublication = api.publication.créer.useMutation({
    onSuccess: (publicationCréée) =>
      publicationCréée && succèsCallback?.(publicationCréée),
    onError: (error) => {
  ...
  const créerPublication: SubmitHandler<PublicationFormulaireInputs> = (
    data,
  ) => {
