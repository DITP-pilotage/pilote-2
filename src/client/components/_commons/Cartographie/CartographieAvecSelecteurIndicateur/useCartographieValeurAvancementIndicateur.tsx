...
export function useCartographieValeurAvancementIndicateur(
  ...
  const useRecupererDonnees = () => {
    const donnees = objectEntries(detailsIndicateurTerritoire).map(
      ([territoireCode, détailsIndicateur]) => ({
        valeur: détailsIndicateur.valeurAvancement ?? null,
    ...
    const tousApplicables: Boolean = donnees.every(
      (donnee) => donnee.estApplicable,
    );
    const tousNonNull: Boolean = donnees.every(
      (donnee) => donnee.valeur !== null,
    );

    let legende = Object.values(élémentsDeLégende);
    if (tousApplicables) {
      legende = legende.filter(
        (el) =>
          el.libellé !==
    ...
    const legendeDegrade = {
      libellé:
        unité === null || unité == undefined
          ? ""
          : `En ${unité.toLocaleLowerCase()}`,
