...
export default function useInputGroupe({
  ...
  const trierLesOptions = useCallback(() => {
    let optionsGroupéesTriées = JSON.parse(JSON.stringify(optionsGroupées)) as InputGroupeProps['optionsGroupées'];

    optionsGroupéesTriées.forEach((groupe, index) => {
      const optionsSélectionnées = groupe.options.filter(option => valeurSelectionneeParDefaut === option.value);
      const optionsNonSélectionnées = groupe.options.filter(option => !(valeurSelectionneeParDefaut === option.value));
  ...
  const filtrerLesOptions = useCallback(() => {
    let optionsGroupéesQuiCorrespondentÀLaRecherche = JSON.parse(JSON.stringify(optionsGroupées)) as InputGroupeProps['optionsGroupées'];
