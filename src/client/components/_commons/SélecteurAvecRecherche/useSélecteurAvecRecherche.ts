...
import SélecteurAvecRechercheProps, {
  SélecteurAvecRechercheOption,
} from "./SélecteurAvecRecherche.interface";
...
export default function useSélecteurAvecRecherche<T extends string>(
  ...
  const [optionsFiltrées, setOptionsFiltrées] =
    useState<SélecteurAvecRechercheOption<T>[]>();
  const [recherche, setRecherche] = useState("");

  const determinerLibellé = useCallback(() => {
    return options.find((option) => option.valeur === valeurSélectionnée)
      ?.libellé;
  }, [options, valeurSélectionnée]);

  const filtrerLesOptions = useCallback(() => {
    const optionsQuiCorrespondentÀLaRecherche = options.filter((option) =>
      rechercheUnTexteContenuDansUnContenant(recherche, option.libellé),
    );
  ...
  useEffect(() => {
    ...
    else setOptionsFiltrées(options);
