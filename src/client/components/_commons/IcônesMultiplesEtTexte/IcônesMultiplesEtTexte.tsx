...
const IcônesMultiplesEtTexte: FunctionComponent<
  IcônesMultiplesEtTexteProps
> = ({
  icônesId,
    ...
    <IcônesMultiplesEtTexteStyled largeurDesIcônes={largeurDesIcônes}>
      <span className="icônes text-lg fr-pr-1w">
        {icônesId.map((icôneId) => (
          <Icône id={icôneId} key={icôneId} />
        ))}
        {!!texteAlternatifPourIcônes && (
          <span className="fr-sr-only">{texteAlternatifPourIcônes}</span>
        )}
      </span>
