...
const MATERIAL_SYMBOLS_CLASSES_CSS_À_PARTIR_DE_LA_VARIANTE: Record<
  string,
  string
> = {
...
const MATERIAL_ICONS_CLASSES_CSS_À_PARTIR_DE_LA_VARIANTE: Record<
  string,
  string
> = {
...
const Icône: FunctionComponent<IcôneProps> = ({ id, className }) => {
  ...
  if (bibliothèqueDIcônes === "dsfr") {
    const variante = DSFR_ICONS_VARIANTES.has(varianteExplicite)
      ? varianteExplicite
      : "fill";
    return <IcôneStyled className={`fr-icon-${nomIcône}-${variante}`} />;
  }

  if (bibliothèqueDIcônes === "material-symbols") {
    const variante =
      MATERIAL_SYMBOLS_CLASSES_CSS_À_PARTIR_DE_LA_VARIANTE[varianteExplicite] ??
      MATERIAL_SYMBOLS_CLASSES_CSS_À_PARTIR_DE_LA_VARIANTE.outlined;

    return (
      <IcôneStyled className={`${variante}${className ? ` ${className}` : ""}`}>
        {nomIcône}
      </IcôneStyled>
    );
  }
  ...
  if (bibliothèqueDIcônes === "material-icons") {
    const variante =
      MATERIAL_ICONS_CLASSES_CSS_À_PARTIR_DE_LA_VARIANTE[varianteExplicite] ??
  ...
  if (bibliothèqueDIcônes === "remix") {
    const variante = REMIX_ICONS_VARIANTES.has(varianteExplicite)
      ? varianteExplicite
      : "fill";
    return <IcôneStyled className={`ri-${nomIcône}-${variante}`} />;
  }
