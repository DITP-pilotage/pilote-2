...
const ExportDesDonnées: FunctionComponent<{ listeChantierId: string[] }> = ({
  listeChantierId,
}) => {
  const [ressourceÀExporter, setRessourceÀExporter] = useState<
    keyof typeof ressources | undefined
  >();
  ...
  if (listeChantierId.length > 0) {
    arrayOptionsExport.push({
      name: "listeChantierId",
      value: listeChantierId.join(","),
    });
  }
    ...
    >
      <form
        ...
        onSubmit={(événement) => {
          ...
          if (!ressourceÀExporter) return;

          const { url, baseDuNomDeFichier, options } =
            ressources[ressourceÀExporter];
          if (url) {
      ...
      >
        ...
        >
          ...
          {Object.values(ressources).map((ressource) => (
            <div className="fr-fieldset__element" key={ressource.id}>
              <div className="fr-radio-group">
                <input
                  ...
                  type="radio"
                />
                <label className="fr-label" htmlFor={ressource.id}>
                  {ressource.libellé}
                </label>
              </div>
            </div>
        ...
        <ul className="fr-btns-group fr-btns-group--left fr-btns-group--inline-sm fr-btns-group--icon-left">
          <li>
            <button
