...
const TableauAdminUtilisateurs: FunctionComponent<{
  ...
  const [typeCompte, setTypeCompte] = useQueryState('typeCompte', parseAsString.withDefault('actif,desactive').withOptions({
    shallow: false,
    clearOnDefault: true,
    history: 'push',
  }));

  const modifierFiltre = (typeCompteAAfficher: ['actif', 'desactive'] | ['actif'] | ['desactive']) => {
    setPagination({
      pageIndex: 1,
    });
  ...
  const verifierTypeEstPresent = (regex: RegExp, typeExport: string) => regex.test(typeExport);
