...
export const ExportDesDonnees: FunctionComponent<{}> = () => {
  ...
  const arrayOptionsExport: {
    ...
    ...filtres.territoires.split(',').filter(Boolean).map(filtreTerritoire => ({
      name: 'territoires',
      value: filtreTerritoire,
    })),
    ...filtres.perimetresMinisteriels.split(',').filter(Boolean).map(filtrePerimetreMinisteriel => ({
      name: 'perimetresMinisteriels',
      value: filtrePerimetreMinisteriel,
    })),
    ...filtres.chantiers.split(',').filter(Boolean).map(filtreChantier => ({
      name: 'chantiers',
      value: filtreChantier,
    })),
    ...filtres.profils.split(',').filter(Boolean).map(filtreProfil => ({
      name: 'profils',
      value: filtreProfil,
    })),
    ...filtres.typeCompte.split(',').filter(Boolean).map(filtreTypeCompte => ({
      name: 'typeCompte',
