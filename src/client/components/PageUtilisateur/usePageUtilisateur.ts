...
export default function usePageUtilisateur(utilisateur: Utilisateur) {
  ...
  const fermerLaModaleDeDesactivationUtilisateur = () => {
    if (typeof window.dsfr === "function") {
      window
        .dsfr(document.querySelector<HTMLElement>("#desactiver-compte"))
        ?.modal?.conceal();
    }
  };

  const desactiverUtilisateur = () => {
    mutationDesactiverUtilisateur.mutate({
  ...
  const fermerLaModaleDeReactivationUtilisateur = () => {
    if (typeof window.dsfr === "function") {
      window
        .dsfr(document.querySelector<HTMLElement>("#reactiver-compte"))
        ?.modal?.conceal();
    }
  };

  const reactiverUtilisateur = () => {
    mutationReactiverUtilisateur.mutate({
      email: utilisateur.email,
      csrf: récupérerUnCookie("csrf") ?? "",
    });
  };

  const modificationEstImpossible = (
    session: Session | null,
    utilisateurHabilitations: Habilitations,
    ...
    if (
      [ProfilEnum.DITP_ADMIN, ProfilEnum.DITP_PILOTAGE].includes(session.profil)
    ) {
      return false;
    }
    ...
    return (
      (!habilitations.peutAccéderAuxTerritoiresUtilisateurs(
        utilisateurHabilitations.lecture.territoires,
      ) &&
        [
          ProfilEnum.COORDINATEUR_REGION,
          ProfilEnum.COORDINATEUR_DEPARTEMENT,
        ].includes(session.profil)) ||
      (!habilitations.peutAccéderAuxChantiersUtilisateurs(
        utilisateurHabilitations.lecture.chantiers,
      ) &&
        session.profil === ProfilEnum.SECRETARIAT_GENERAL) ||
      [
        ProfilEnum.COORDINATEUR_REGION,
        ProfilEnum.COORDINATEUR_DEPARTEMENT,
      ].includes(utilisateurProfil)
    );
  };

  const donnneContenuBandeau = (
    session: Session | null,
    utilisateurHabilitations: Habilitations,
    utilisateurProfil: ProfilCode,
  ) => {
  ...
  const habilitationsAGenererUnTokenDAuthentification = (
    session: Session,
    utilisateurProfil: ProfilCode,
  ) => {
    ...
    return !(
      !session ||
      !PROFIL_ADMIN_AUTORISE_A_MODIFIER.has(session.profil) ||
      !PROFIL_AUTORISE_A_POSSEDER_UN_TOKEN_API.has(utilisateurProfil)
    );
  };
