...
const ORDRE_DES_TENDANCE: ChantierTendance[] = [
  "BAISSE",
  "HAUSSE",
  "STAGNATION",
];
...
export function comparerTendance(
  tendanceA: ChantierTendance | null,
  tendanceB: ChantierTendance | null,
  tri: SortingState,
) {
  const sensDeTriDesc = tri[0].desc;
  if (tendanceA === null && tendanceB === null) return 0;
  if (tendanceA === null) return sensDeTriDesc ? -1 : 1;
  if (tendanceB === null) return sensDeTriDesc ? 1 : -1;
