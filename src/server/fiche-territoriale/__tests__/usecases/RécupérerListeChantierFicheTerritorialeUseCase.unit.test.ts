...
describe("RécupérerListeChantierFicheTerritorialeUseCase", () => {
  ...
  it("quand le territoire est un département, doit récupérer la liste des chantiers associés trié par taux avancement", async () => {
    ...
    const territoire = new TerritoireBuilder()
      .withCodeInsee("34")
      .withMaille("DEPT")
      .build();
    ...
    const syntheseDesResultats1CH3 = new SyntheseDesResultatsBuilder()
      ...
      .withDateMeteo("2022-01-02T00:00:00.000Z")
      .build();
    mapSyntheseDesResultats.set("CH-001", [
      syntheseDesResultats1CH1,
      syntheseDesResultats2CH1,
    ]);
    mapSyntheseDesResultats.set("CH-002", [
      syntheseDesResultats1CH2,
      syntheseDesResultats2CH2,
    ]);
    ...
    const ministere1 = new MinistereBuilder()
      .withCode("1009")
      .withIcone("remix::football::fill")
      .build();
    const ministere2 = new MinistereBuilder()
      .withCode("10")
      .withIcone("remix::basket::fill")
      .build();
    ...
    syntheseDesResultatsRepository.recupererMapSyntheseDesResultatsParListeChantierIdEtTerritoire.mockResolvedValue(
      mapSyntheseDesResultats,
    );
    indicateurRepository.recupererMapIndicateursParListeChantierIdEtTerritoire.mockResolvedValue(
      mapIndicateurs,
    );
    indicateurRepository.recupererMapIndicateursNationalParListeIndicateurId.mockResolvedValue(
      mapIndicateursNational,
    );
    ministereRepository.recupererMapMinistereParListeCodeMinistere.mockResolvedValue(
      mapMinistere,
    );
    ...
    // When
    const result = await récupérerListeChantierFicheTerritorialeUseCase.run({
      territoireCode,
      jalon,
    });
    ...
    expect(
      syntheseDesResultatsRepository.recupererMapSyntheseDesResultatsParListeChantierIdEtTerritoire,
    ).toHaveBeenNthCalledWith(1, {
      listeChantierId: ["CH-002", "CH-003", "CH-001"],
      maille: "DEPT",
      codeInsee: "34",
    });
    expect(
      indicateurRepository.recupererMapIndicateursParListeChantierIdEtTerritoire,
    ).toHaveBeenNthCalledWith(1, {
      listeChantierId: ["CH-002", "CH-003", "CH-001"],
      maille: "DEPT",
      codeInsee: "34",
    ...
    expect(
      indicateurRepository.recupererMapIndicateursNationalParListeIndicateurId,
    ).toHaveBeenNthCalledWith(1, {
      listeIndicateurId: [
        "IND-001",
        "IND-002",
    ...
    expect(
      ministereRepository.recupererMapMinistereParListeCodeMinistere,
    ).toHaveBeenNthCalledWith(1, { listeCodeMinistere: ["10", "10", "1009"] });
    ...
    expect(result.at(0)?.indicateurs.at(0)?.nom).toEqual(
      "Un nom indicateur 1 chantier 2",
    );
    ...
    expect(result.at(0)?.indicateurs.at(0)?.tauxAvancementNational).toEqual(
      30.1,
    );
    expect(result.at(0)?.indicateurs.at(0)?.uniteMesure).toEqual(
      "million habitant",
    );
    expect(result.at(0)?.indicateurs.at(1)?.nom).toEqual(
      "Un nom indicateur 2 chantier 2",
    );
    ...
    expect(result.at(0)?.indicateurs.at(1)?.tauxAvancementNational).toEqual(
      40.1,
    );
    expect(result.at(0)?.indicateurs.at(1)?.uniteMesure).toEqual(
      "million habitant",
    );

    expect(result.at(1)?.tauxAvancement).toEqual(25);

    expect(result.at(2)?.nom).toEqual(
      "Poursuivre le déploiement du Pass Culture",
    );
    expect(result.at(2)?.ministerePorteur.icone).toEqual(
      "remix::football::fill",
    );
    ...
    expect(result.at(2)?.indicateurs.at(0)?.nom).toEqual(
      "Un nom indicateur 1 chantier 1",
    );
    ...
    expect(result.at(2)?.indicateurs.at(0)?.tauxAvancementNational).toEqual(
      10.1,
    );
    expect(result.at(2)?.indicateurs.at(0)?.uniteMesure).toEqual("Pourcentage");
    expect(result.at(2)?.indicateurs.at(1)?.nom).toEqual(
      "Un nom indicateur 2 chantier 1",
    );
    ...
    expect(result.at(2)?.indicateurs.at(1)?.tauxAvancementNational).toEqual(
      20.1,
    );
  ...
  it("quand le territoire est une région, doit récupérer la liste des chantiers associés", async () => {
    ...
    expect(
      ministereRepository.recupererMapMinistereParListeCodeMinistere,
    ).toHaveBeenNthCalledWith(1, { listeCodeMinistere: ["1009", "10"] });
    expect(result).toHaveLength(2);

    expect(result.at(0)?.nom).toEqual(
      "Poursuivre le déploiement du Pass Culture",
    );
    expect(result.at(0)?.ministerePorteur.icone).toEqual(
      "remix::football::fill",
    );
