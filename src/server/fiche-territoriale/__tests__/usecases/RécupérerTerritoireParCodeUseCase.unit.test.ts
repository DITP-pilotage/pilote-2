...
describe("RécupérerTerritoireParCodeUseCase", () => {
  ...
  beforeEach(() => {
    ...
    récupérerTerritoireParCodeUseCase = new RécupérerTerritoireParCodeUseCase({
  ...
  test("doit récupérer le territoire associé au code donné", async () => {
    ...
    // When
    const result = await récupérerTerritoireParCodeUseCase.run({
      territoireCode,
    });
    ...
    ).toHaveBeenNthCalledWith(1, { territoireCode: "UN-CODE" });
