...
export class PrismaChantierRepository implements ChantierRepository {
  async listerParTerritoireCodePourUnDepartement({
    ...
    return this.listerParTerritoireCodePourEtMaille({
      territoireCode,
      maille: "DEPT",
      jalon,
    });
  ...
  async listerParTerritoireCodePourUneRegion({
    territoireCode,
    jalon,
  }: {
    territoireCode: string;
  ...
  async listerParTerritoireCodePourEtMaille({
    territoireCode,
    maille,
    jalon,
  }: {
  ...
  private convertirEnChantier(
    prismaChantierTerritoireModel: PrismaChantierTerritoireModel & {
      chantier_identite: { nom: string; ministeres: string[] };
      chantier_territoire_jalon: { taux_avancement: number | null }[];
    },
  ): Chantier {
    return Chantier.creerChantier({
      ...
      codeMinisterePorteur:
        prismaChantierTerritoireModel.chantier_identite.ministeres.at(0),
    });
  }
}
