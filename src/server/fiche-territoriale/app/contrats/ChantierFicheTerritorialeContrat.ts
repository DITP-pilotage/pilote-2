...
const presenterEnIndicateurFicheTerritorialeContrat = (
  indicateur: IndicateurFicheTerritoriale,
): IndicateurFicheTerritorialeContrat => ({
  nom: indicateur.nom,
...
export const presenterEnChantierFicheTerritorialeContrat = (
  chantier: ChantierFicheTerritoriale,
): ChantierFicheTerritorialeContrat => ({
  nom: chantier.nom,
  meteo: chantier.meteo || "NON_RENSEIGNEE",
  tauxAvancement: chantier.tauxAvancement,
  dateQualitative: chantier.dateQualitative
    ? `(${formaterDate(chantier.dateQualitative, "MM/YYYY")})`
    : "",
  dateQuantitative: chantier.dateQuantitative
    ? `(${formaterDate(chantier.dateQuantitative, "MM/YYYY")})`
    : "",
  ministereIcone: chantier.ministerePorteur.icone,
  indicateurs: chantier.indicateurs.map(
    presenterEnIndicateurFicheTerritorialeContrat,
  ),
});
