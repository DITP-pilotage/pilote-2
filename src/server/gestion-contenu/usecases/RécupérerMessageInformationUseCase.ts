...
export class RécupérerMessageInformationUseCase {
  ...
  async run(): Promise<MessageInformation> {
    const listeVariableContenu =
      await this._gestionContenuRepository.recupererMapVariableContenuParListeDeNom(
        [
    ...
    return MessageInformation.creerMessageInformation({
      bandeauTexte:
        listeVariableContenu.NEXT_BD_FF_BANDEAU_INDISPONIBILITE_TEXTE!, // trouver un meilleur type pour enlever les !!!
      isBandeauActif: listeVariableContenu.NEXT_BD_FF_BANDEAU_INDISPONIBILITE!,
      bandeauType:
        listeVariableContenu.NEXT_BD_FF_BANDEAU_INDISPONIBILITE_TYPE!,
    });
  }
}
