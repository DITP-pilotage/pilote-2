...
const convertirEnInformationChantierUtilisateur = (
  prismaChantier: Pick<
    PrismaChantierIdentiteModel,
    "id" | "nom" | "est_territorialise" | "perimetre_ids" | "statut" | "ate"
  > & {
    chantier_territoire: Pick<
      PrismaChantierTerritoireModel,
      "territoire_code"
    >[];
  },
  ...
  return {
    ...
    territoiresApplicables: prismaChantier.chantier_territoire.map(
...
export class PrismaChantierRepository implements ChantierRepository {
  ...
  async listerInformationsChantiersUtilisateurs(): Promise<
    InformationChantierUtilisateur[]
  > {
