...
const convertirEnPerimetreMinisteriel = (périmètre: PerimetreMinisterielPrisma): PerimetreMinisteriel => {
...
export class PrismaPerimetreMinisterielRepository implements PerimetreMinisterielRepository {
  ...
  async listerIds(listePerimetresMinisterielsIds: string[]): Promise<string[]> {
    ...
    return listePrismaPerimetreMinisteriel.map(perimetreMinisteriel => perimetreMinisteriel.id);
  }
