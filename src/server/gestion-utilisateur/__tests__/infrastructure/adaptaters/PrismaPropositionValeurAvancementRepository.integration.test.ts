...
describe('PrismaObjectifRepository', () => {
  ...
  beforeEach(() => {
    prismaPropositionValeurAvancement = new PrismaPropositionValeurAvancementRepository();
  });
  describe('#anonymiserAuteurs', () => {
    test('doit anonymiser l\'auteur des objectifs saisis par l\'utilisateur supprimé', async () => {
      ...
      expect(propositionsAvecAuteurAnonyme[0].id).toStrictEqual('b699907e-43c4-43be-8d8d-185fca1b2e50');
      expect(propositionsAvecAuteurAnonyme[1].id).toStrictEqual('e3885e40-caab-4fb6-acf4-0c8f66c9e290');
    });
  });
});
