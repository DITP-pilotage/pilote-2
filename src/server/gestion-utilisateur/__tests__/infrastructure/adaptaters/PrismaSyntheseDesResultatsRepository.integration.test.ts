...
describe('PrismaSyntheseDesResultatsRepository', () => {
  ...
  describe('#anonymiserAuteurs', () => {
    test('doit anonymiser l\'auteur des synthèses saisies par l\'utilisateur supprimé', async () => {
      // Given
      ...
      // When
      await prismaSyntheseDesResultatsRepository.anonymiserAuteurs([auteurId2], 'utilisateur.supprime@modernisation.gouv.fr');

      // Then
      const synthesesAvecAuteurAnonyme = await prisma.synthese_des_resultats.findMany({ where: { auteur_id: auteurId3 } });
      expect(synthesesAvecAuteurAnonyme).toHaveLength(2);
      expect(synthesesAvecAuteurAnonyme[0].id).toStrictEqual('b699907e-43c4-43be-8d8d-185fca1b2e50');
      expect(synthesesAvecAuteurAnonyme[1].id).toStrictEqual('e3885e40-caab-4fb6-acf4-0c8f66c9e290');
    });
  });
});
