...
describe("PrismaUtilisateurRepository", () => {
  ...
  describe("récupérerNombreUtilisateursParTerritoires", function () {
    test("retourne les nombres d'utilisateurs pour une liste de territoires", async () => {
      ...
      const utilisateursCréés = await prisma
        .getInstance()
  ...
  describe("desactiver", function () {
    ...
    test("Si l'email n'existe pas, ne fait rien", async () => {
      ...
      // When
      await utilisateurRepository.desactiver(
        "utilisateurinexistant@test.com",
        auteurId,
      );

      // Then
      const utilisateurNonExistant = await prisma
        .getInstance()
      ...
      const utilisateurExistant = await prisma
        .getInstance()
        .utilisateur.findFirst({
          where: {
            email: "utilisateuracreer@test.com",
    ...
    test("Si l'email existe, mets à jour la date de desactivation, la date de dernière modification et l\'auteur modification", async () => {
      ...
      // When
      await utilisateurRepository.desactiver(
        "utilisateuracreer@test.com",
        auteurId,
      );
      const utilisateurDesactive = await prisma
        .getInstance()
        .utilisateur.findFirst({
          where: {
            email: "utilisateuracreer@test.com",
  ...
  describe("reactiver", function () {
    ...
    test("Si l'email n'existe pas, ne fait rien", async () => {
      ...
      // When
      await utilisateurRepository.reactiver(
        "utilisateurinexistant@test.com",
        auteurId,
      );
      ...
      expect(utilisateurNonExistant).toBeNull();
      expect(utilisateurExistant?.date_desactivation).toStrictEqual(
        dateDesactivation,
      );
    });
