...
describe("PrismaRapportRepository", () => {
  ...
  describe("#anonymiserAuteurs", () => {
    test("doit anonymiser l'auteur des rapports importés par l'utilisateur supprimé", async () => {
      ...
      // When
      await prismaRapportRepository.anonymiserAuteurs(
        ["auteur.import@test.com"],
        "utilisateur.supprime@modernisation.gouv.fr",
      );

      // Then
      const rapportsAvecAuteurAnonyme =
      ...
      expect(rapportsAvecAuteurAnonyme[0].id).toStrictEqual(
        "b699907e-43c4-43be-8d8d-185fca1b2e50",
      );
      expect(rapportsAvecAuteurAnonyme[1].id).toStrictEqual(
