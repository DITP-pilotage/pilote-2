...
describe('AjouterUnContactAUneInfoLettreUseCase', () => {
  ...
  beforeEach(() => {
    ...
    ajouterUnContactAUneInfoLettreUseCase = new AjouterUnContactAUneInfoLettreUseCase({
  ...
  it("Si l'id utilisateur n'existe pas en base, retourne faux et n'ajoute aucun contact à l'infolettre", async () => {
    ...
    // When
    const contactAjoute = await ajouterUnContactAUneInfoLettreUseCase.execute('9b62a20d-f344-4b7b-939a-2c61db177c4d', 10);

    // Then
    expect(contactInfoLettresService.ajouterContactAUneInfoLettre).toHaveBeenCalledTimes(0);
    expect(utilisateurRepository.mettreAJourLaDateInscriptionInfolettre).toHaveBeenCalledTimes(0);
  ...
  it("Si l'id utilisateur existe en base, retourne true et ajoute le contact à l'infolettre", async () => {
    // Given
    utilisateurRepository.recupererUtilisateurEmail.mockResolvedValue('john.doe@exemple.com');

    // When
    const contactAjoute = await ajouterUnContactAUneInfoLettreUseCase.execute('9b62a20d-f344-4b7b-939a-2c61db177c4d', 10);

    // Then
    expect(contactInfoLettresService.ajouterContactAUneInfoLettre).toHaveBeenCalledWith('john.doe@exemple.com', [10]);
    expect(utilisateurRepository.mettreAJourLaDateInscriptionInfolettre).toHaveBeenCalledWith('john.doe@exemple.com', new Date('2025-06-17T08:06:12.411Z'));
