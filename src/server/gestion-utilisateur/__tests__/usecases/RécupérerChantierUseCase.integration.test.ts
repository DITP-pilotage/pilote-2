...
import PérimètreMinistérielSQLRowBuilder from "@/server/infrastructure/test/builders/sqlRow/PérimètreMinistérielSQLRow.builder";
...
describe("RécupérerChantierUseCase", () => {
  ...
  beforeEach(() => {
    ...
    récupérerChantierUseCase = new RécupérerChantierUseCase(
      chantierRepository,
      ministèreRepository,
  ...
  test("Accède à un chantier par son id, vérification de quelques champs", async () => {
      ...
      data: [
        {
          ...
          directeurs_projet_mails: [
            "dirproj1@example.com",
            "dirproj2@example.com",
          ],
    ...
    // When
    const result1 = await récupérerChantierUseCase.run(
      "CH-001",
      habilitation,
      profil,
      jalon,
    );
    const result2 = await récupérerChantierUseCase.run(
      "CH-002",
      habilitation,
      profil,
      jalon,
    ...
    expect(result1.responsables.directeursAdminCentrale).toStrictEqual([
      { nom: "Alain Térieur", direction: "Intérieur" },
      { nom: "Alex Térieur", direction: "Extérieur" },
    ]);
    expect(result1.responsables.directeursProjet).toStrictEqual([
      { nom: "Dir proj 1", email: "dirproj1@example.com" },
      { nom: "Dir proj 2", email: "dirproj2@example.com" },
    ]);
  ...
  test("un chantier contenant une maille nationale et départementale", async () => {
    ...
    // When
    const result = await récupérerChantierUseCase.run(
      chantierId,
      habilitation,
      profil,
      jalon,
  ...
  test("Contient des porteurs et des coporteurs", async () => {
    // Given
    const ministères = [
      new MinistèreSQLRowBuilder()
        .avecId("1")
        .avecNom("Agriculture et Alimentation")
        .build(),
    ...
    await prisma.perimetre.createMany({
      data: [
        new PérimètreMinistérielSQLRowBuilder()
          .avecMinistère(ministères[0])
          .build(),
        new PérimètreMinistérielSQLRowBuilder()
          .avecMinistère(ministères[1])
          .build(),
        new PérimètreMinistérielSQLRowBuilder()
          .avecMinistère(ministères[2])
          .build(),
      ],
    });
    ...
    expect(result1.responsables.porteur!.nom).toEqual(
      "Agriculture et Alimentation",
    );
    expect(result1.responsables.coporteurs).toBeDefined();
    expect(result1.responsables.coporteurs.map((cp) => cp.nom)).toEqual([
      "Intérieur",
      "Extérieur",
    ]);
  ...
  test("Un directeur de projet peut ne pas avoir d'adresse email", async () => {
    ...
    // Then
    expect(result.responsables.directeursProjet[0]).toStrictEqual({
      nom: "Jean Bon",
      email: null,
    });
  });
  ...
  describe("Gestion d'erreur", () => {
    test("Erreur en cas d'absence de maille nationale", async () => {
      ...
      // When
      const request = async () => {
        await récupérerChantierUseCase.run(
          chantierId,
          habilitation,
          profil,
          jalon,
      ...
      await expect(request).rejects.toThrow(
        /le chantier 'CH-001' n'a pas de maille nationale/,
      );
    });
  });
});
