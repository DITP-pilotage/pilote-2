...
export class SupprimerLesComptesDesactivesUseCase {
  ...
  async run(): Promise<{ id: string, email: string }[]> {
    ...
    const { listeUtilisateurASupprimerIds, listeUtilisateurASupprimerEmails } = utilisateursInactifs.reduce((acc, utilisateur) => {
      ...
      return acc;
    }, { listeUtilisateurASupprimerIds: [] as string[], listeUtilisateurASupprimerEmails: [] as string[] });    
    await this.commentaireRepository.anonymiserAuteurs(listeUtilisateurASupprimerIds, EMAIL_AUTEUR_REMPLACEMENT);
    await this.syntheseDesResultatsRepository.anonymiserAuteurs(listeUtilisateurASupprimerIds, EMAIL_AUTEUR_REMPLACEMENT);
    await this.decisionStrategiqueRepository.anonymiserAuteurs(listeUtilisateurASupprimerIds, EMAIL_AUTEUR_REMPLACEMENT);
