  ...
  describe('quand le fichier est invalide', () => {
    ...
    describe('et que le rapport possèdes des erreurs spécifiques, doit personnaliser le message', () => {
      it.each([
        ...
        ['type_valeur', 'constraint-error', 'A field value does not conform to a constraint.', "constraint \"enum\" is \"['vi', 'va', 'vc']\"", 1, 2, 'Le type de valeur doit être vi (valeur initiale), va (valeur d’avancement) ou vc (valeur cible).'],
        ['type_valeur', 'constraint-error', 'A field value does not conform to a constraint.', "constraint \"enum\" is \"['va']\"", 1, 2, 'Le type de valeur doit être va (valeur d’avancement). Vous ne pouvez saisir que des valeurs d’avancement.'],
