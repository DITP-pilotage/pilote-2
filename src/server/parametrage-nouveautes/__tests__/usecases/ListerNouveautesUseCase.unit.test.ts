...
describe('ListerNouveautesUseCase', () => {
  ...
  beforeEach(() => {
    ...
    listerNouveautesUseCase = new ListerNouveautesUseCase({ nouveauteRepository });
  });

  it('Doit lister les nouveautés', async () => {
    // Given
    const nouveautes = [
      Nouveaute.creerNouveaute({ contenu: 'Nouvelle nouveauté', version: '1.0.0', date: new Date().toISOString() }),
      Nouveaute.creerNouveaute({ contenu: 'Nouvelle nouveauté 2', version: '1.0.1', date: new Date().toISOString() }),
    ];
