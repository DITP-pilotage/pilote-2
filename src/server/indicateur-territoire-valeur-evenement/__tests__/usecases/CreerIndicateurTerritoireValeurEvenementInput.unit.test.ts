...
describe('CreerIndicateurTerritoireValeurEvenementUseCase', () => {
  ...
  beforeEach(() => {
    indicateurTerritoireValeurEvenementRepository = mock<IndicateurTerritoireValeurEvenementRepository>();
    creerIndicateurTerritoireValeurEvenementUseCase = new CreerIndicateurTerritoireValeurEvenementUseCase({
  ...
  it('Doit créer un événement indicateur territoire valeur avec les bonnes propriétés', async () => {
    ...
    // Then
    expect(indicateurTerritoireValeurEvenementRepository.enregistrer).toHaveBeenCalledWith(
  ...
  it('Doit appeler le repository une seule fois', async () => {
    ...
    // Then
    expect(indicateurTerritoireValeurEvenementRepository.enregistrer).toHaveBeenCalledTimes(1);
  });
  ...
  it('Doit générer un correlationId et un id unique pour chaque appel', async () => {
    ...
    // Then
    expect(indicateurTerritoireValeurEvenementRepository.enregistrer).toHaveBeenCalledTimes(2);
    
    const firstCall = indicateurTerritoireValeurEvenementRepository.enregistrer.mock.calls[0][0];
    const secondCall = indicateurTerritoireValeurEvenementRepository.enregistrer.mock.calls[1][0];
  ...
  it('Doit récupérer les événements existants avec les bons paramètres', async () => {
    ...
    // Then
    expect(indicateurTerritoireValeurEvenementRepository.recupererParIndicIdTerritoireCodeTypeValeurEtDate).toHaveBeenCalledWith({
      indicId: input.indicId,
      territoireCode: input.territoireCode,
      typeValeur: 'VALEUR_AVANCEMENT',
      dateValeur: input.dateValeurAvancement,
  ...
  it('Doit calculer le prochain ordre quand des événements existent', async () => {
    ...
    indicateurTerritoireValeurEvenementRepository.recupererParIndicIdTerritoireCodeTypeValeurEtDate.mockResolvedValue(evenementsExistants);
