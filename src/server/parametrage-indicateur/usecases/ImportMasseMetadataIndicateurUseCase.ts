...
const convertirEnImportMetadataIndicateur = (
  record: RecordCSVImport,
  ...
  indicIsPerseverant:
    (record.indic_is_perseverant &&
      record.indic_is_perseverant.toLowerCase() === "true") ||
    false,
  indicIsPhare:
    (record.indic_is_phare && record.indic_is_phare.toLowerCase() === "true") ||
    false,
  indicIsBaro:
    (record.indic_is_baro && record.indic_is_baro.toLowerCase() === "true") ||
    false,
  ...
  indicHiddenPilote:
    (record.indic_hidden_pilote &&
      record.indic_hidden_pilote.toLowerCase() === "true") ||
    false,
  ...
  poidsPourcentEvalNat: record.poids_pourcent_eval_nat_declaree || "0",
  poidsPourcentEvalReg: record.poids_pourcent_eval_reg_declaree || "0",
  poidsPourcentEvalDept: record.poids_pourcent_eval_dept_declaree || "0",
  tendance: record.tendance,
  reformePrioritaire: record.reforme_prioritaire || null,
  projetAnnuelPerf:
    (record.projet_annuel_perf &&
      record.projet_annuel_perf.toLowerCase() === "true") ||
    false,
  detailProjetAnnuelPerf: record.detail_projet_annuel_perf || null,
  periodicite: record.periodicite,
  delaiDisponibilite: !Number.isNaN(record.delai_disponibilite)
    ? Number(record.delai_disponibilite)
    : 0,
  indicTerritorialise:
    (record.indic_territorialise &&
      record.indic_territorialise.toLowerCase() === "true") ||
    false,
  frequenceTerritoriale: !Number.isNaN(record.frequence_territoriale)
    ? Number(record.frequence_territoriale)
    : 0,
  ...
  donneeOuverte:
    (record.donnee_ouverte && record.donnee_ouverte.toLowerCase() === "true") ||
    false,
  ...
  cibleAttendue:
    (record.cible_attendue && record.cible_attendue.toLowerCase() === "true") ||
    false,
...
export default class ImportMasseMetadataIndicateurUseCase {
  ...
  async run({ nomDuFichier }: { nomDuFichier: string }) {
    ...
    const listeRecordCsvImport: RecordCSVImport[] = parse(
      contents,
      csvParserOptions,
    );
    ...
    if (listeRecordCsvImport.length > 0) {
      if (
        Object.keys(listeRecordCsvImport[0]).sort().toString() ===
        Object.values(AvailableHeaderCSVImport).sort().toString()
      ) {
        let identifiants: Set<number> = new Set();
        let identifiantsFichier: Set<number> = new Set();
        if (
          listeRecordCsvImport.some(
            (record) => record.indic_id === TEXT_LABEL_CREATION_ID,
          )
        ) {
          const listeMetadataParametrageIndicateur =
            await this._metadataParametrageIndicateurRepository.recupererListeMetadataParametrageIndicateurEnFonctionDesFiltres(
              [],
              [],
              false,
              false,
          ...
          const sortedListeMetadataParametrageIndicateur =
            listeMetadataParametrageIndicateur.sort(
              (
                metadataParametrageIndicateur1,
                metadataParametrageIndicateur2,
              ) =>
          ...
          identifiants = new Set(
            sortedListeMetadataParametrageIndicateur.map(
              (metadataParametrageIndicateur) =>
                Number(metadataParametrageIndicateur.indicId.split("-")[1]),
            ),
          );
          identifiantsFichier = new Set(
            listeRecordCsvImport.map((recordFichier) =>
              Number((recordFichier.indic_id || "").split("-")[1]),
            ),
          );
        }
        let currentId = 1;
        const listeMetadataIndicateur: ImportMetadataIndicateur[] =
          listeRecordCsvImport.map((record) => {
            let indicId = record.indic_id;

            if (indicId === TEXT_LABEL_CREATION_ID) {
              while (
                identifiants.has(currentId) ||
                identifiantsFichier.has(currentId)
              ) {
                currentId++;
              }
            ...
            const importMetadataIndicateur =
              convertirEnImportMetadataIndicateur({
                ...record,
                indic_id: indicId,
              });
        ...
        await this._metadataParametrageIndicateurRepository.importerEnMasseLesMetadataIndicateurs(
