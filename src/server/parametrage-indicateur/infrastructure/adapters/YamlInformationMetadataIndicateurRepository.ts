...
const convertirEnInformationMetadataIndicateur = (
...
export class YamlInformationMetadataIndicateurRepository
  implements InformationMetadataIndicateurRepository
{
  ...
  récupererInformationMetadataIndicateur(): InformationMetadataIndicateur[] {
    ...
    return result.sources
      .find((source) => source.name === "parametrage_indicateurs")!
      .tables.flatMap((table) => table.columns)
      .filter(Boolean)
      .map(convertirEnInformationMetadataIndicateur);
  }
}
