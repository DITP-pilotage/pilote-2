...
const presenterEnIndicateurExportContrat = (indicateurPourExport: IndicateurPourExport, profil: string, optionsExport: OptionsExport): string[] => {
  ...
  if (optionsExport.listeOptionsExport.includes('cadrage')) {
    donnees.push(
      ...
      Number.isNaN(indicateurPourExport.delaiDisponibilite) ? NON_RENSEIGNEE : `${indicateurPourExport.delaiDisponibilite} mois`,
    );
  }
...
export class ExportCsvDesIndicateursUseCaseV2 {
  ...
  public async *run({ chantierIds, territoireCodes, profil, indicateurChunkSize, optionsExport, jalon }: { chantierIds: string[], territoireCodes: string[], profil: ProfilCode, indicateurChunkSize: number, optionsExport: OptionsExport, jalon: number }): AsyncGenerator<string[][]> {
    for (let i = 0; i < chantierIds.length; i += indicateurChunkSize) {
      ...
      const estAvecCadrage = optionsExport.listeOptionsExport.includes('cadrage');

      const input = partialChantierIds.map(id => this.indicateurRepository.recupererPourExports(id, territoireCodes, jalon, estAvecCadrage).then(listeIndicateurTerritoireExport =>
