...
const presenterEnChantierExportContrat = (
  ...
  const donneesProfil = profilsTerritoriaux.includes(profilCode)
    ...
    : [
        ...donneesCommunes,
        ...donneesCommentairesNationaux,
        ...donneesCommentairesGeneraux,
      ];

  return profilCode === ProfilEnum.DITP_ADMIN
    ? [...donneesProfil, chantierPourExport.statut || NON_RENSEIGNEE]
    : donneesProfil;
};
...
export class ExportCsvDesChantiersUseCase {
  public static readonly NOMS_COLONNES = (
    ...
    const colonnesProfil = profilsTerritoriaux.includes(profilCode)
      ...
      : [
          ...colonnesCommunes,
          ...colonnesCommentairesNationaux,
          ...colonnesCommentairesGeneraux,
        ];

    return profilCode === ProfilEnum.DITP_ADMIN
      ? [...colonnesProfil, "Statut"]
      : colonnesProfil;
  };
    ...
    for (let i = 0; i < chantierIds.length; i += chantierChunkSize) {
      ...
      const input = partialChantierIds.map((id) =>
        this.chantierRepository
          .récupérerPourExports(id, territoireCodes, optionsExport, jalon)
