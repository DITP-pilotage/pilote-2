...
describe("ExportCsvDesIndicateursUseCase", () => {
  ...
  it("Quand on renvoie autant de ligne que le chunk, doit renvoyer toutes les lignes", async () => {
    ...
    const indicateur1 = new IndicateurPourExportBuilder()
      .withNom("Indicateur IND-001")
      .build();
    const indicateur2 = new IndicateurPourExportBuilder()
      .withNom("Indicateur IND-002")
      .build();
    const indicateur3 = new IndicateurPourExportBuilder()
      .withNom("Indicateur IND-003")
      .build();

    indicateurRepository.récupérerPourExports.mockResolvedValueOnce([
      indicateur1,
    ]);
    indicateurRepository.récupérerPourExports.mockResolvedValueOnce([
      indicateur2,
    ...
    const usecase = new ExportCsvDesIndicateursUseCase({
    ...
    // When
    let result: string[][] = [];
    for await (const partialResult of usecase.run({
      chantierIds,
      territoireCodes,
      profil,
      indicateurChunkSize,
  ...
  it("Quand on renvoie plus de ligne que le chunks, doit renvoyer toutes les lignes", async () => {
    ...
    const indicateur3 = new IndicateurPourExportBuilder()
      .withNom("Indicateur IND-003")
      .build();
    const indicateur4 = new IndicateurPourExportBuilder()
      .withNom("Indicateur IND-004")
      .build();
  ...
  it("Quand le profil ne doit pas être visible pour les DROM, ne doit pas les remonter", async () => {
    ...
    // Then
    expect(indicateurRepository.récupérerPourExports).toHaveBeenCalledWith(
      "CH-001",
      territoireCodes,
      jalon,
    );
    ...
    expect(result).toEqual(
      [["Indicateur IND-001"], ["Indicateur IND-002"]].map(
