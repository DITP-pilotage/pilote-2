...
describe('CreerPropositionValeurAvancementUseCase', () => {
  ...
  beforeEach(() => {
    ...
    creerPropositionValeurAvancementUseCase = new CreerPropositionValeurAvancementUseCase({ propositionValeurAvancementRepository });
  });
  ...
  it('doit créer une nouvelle proposition de valeur avancement', async () => {
    ...
    // When
    await creerPropositionValeurAvancementUseCase.run({ indicId, valeurAvancementProposee, territoireCode, dateValeurAvancement, idAuteurModification, auteurModification, dateProposition, motifProposition, sourceDonneeEtMethodeCalcul, statut });
    // Then
    const propositionValeurAvancementCaptor = captor<PropositionValeurAvancement>();
    expect(propositionValeurAvancementRepository.annulePropositionValeurAvancementPrecedente).toHaveBeenCalledWith({ indicId, territoireCode });
    expect(propositionValeurAvancementRepository.creerPropositionValeurAvancement).toHaveBeenNthCalledWith(1, propositionValeurAvancementCaptor);
  });
});
