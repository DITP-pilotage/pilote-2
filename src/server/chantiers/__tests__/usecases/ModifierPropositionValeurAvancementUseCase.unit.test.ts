...
describe('ModifierPropositionValeurAvancementUseCase', () => {
  ...
  beforeEach(() => {
    ...
    modifierPropositionValeurAvancementUseCase = new ModifierPropositionValeurAvancementUseCase({ propositionValeurAvancementRepository, indicateurRepository });
  });
  ...
  it('doit créer une nouvelle proposition de valeur actuelle', async () => {
    ...
    // When
    await modifierPropositionValeurAvancementUseCase.run({ indicId, territoireCode, auteurModification });
    // Then
    expect(propositionValeurAvancementRepository.supprimerPropositionValeurAvancement).toHaveBeenCalledTimes(1);
    expect(indicateurRepository.supprimerPropositionValeurAvancement).toHaveBeenCalledTimes(1);
  });
});
