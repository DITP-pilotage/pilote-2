...
const query = new RecupererAvancementsTerritoiresQuery({
  agregerAvancementsChantiersUseCase: new AgregerAvancementsChantiersUseCase({
    chantierRepository: new ChantierSQLRepository(),
  }),
...
describe("RecupererAvancementsTerritoiresQuery", () => {
  describe("dateTauxAvancementAnnuel", () => {
    it("prend la date la plus récente parmi plusieurs chantiers", async () => {
      // given
      ...
      // when
      const result = await query.run({ chantierIds: ["CH-001", "CH-002"], jalon: 2025 });
    ...
    it("retourne null si aucun chantier n'a de date", async () => {
      // given
  ...
  describe("estApplicable", () => {
    it("retourne false si tous les chantiers sont non applicables", async () => {
      // given
      ...
      // when
      const result = await query.run({ chantierIds: ["CH-004", "CH-005"], jalon: 2025 });
    ...
    it("retourne null si au moins un chantier est applicable", async () => {
      // given
      ...
      // when
      const result = await query.run({ chantierIds: ["CH-006", "CH-007"], jalon: 2025 });
