...
describe("PrismaChantierRepository", () => {
  ...
  describe("#récupérerPourExports", () => {
    describe("Données des chantiers pour l'export CSV", () => {
      it("renvoie les bonnes données dans les bons attributs", async () => {
        ...
        // When
        const result = await prismaChantierRepository.récupérerPourExports(
          chantierId,
          territoireCodesLecture,
          optionsPourExport,
          jalon,
        );
      ...
      it("retourne seulement les données sur les territoires applicables", async () => {
        ...
        // When
        const donneesChantier1 =
          await prismaChantierRepository.récupérerPourExports(
            "CH-001",
            territoireCodesLecture,
            optionsPourExport,
            jalon,
          );
        const donneesChantier2 =
          await prismaChantierRepository.récupérerPourExports(
            "CH-002",
            territoireCodesLecture,
            optionsPourExport,
  ...
  describe("#récupérerPourExportsV2", () => {
    it("aUnTauxAvancementDepartemental est vrai, si le chantier ne possède aucun département applicable", async () => {
      ...
      // When
      const result = await prismaChantierRepository.récupérerPourExportsV2(
        "CH-001",
        ["NAT-FR"],
        optionsPourExport,
        2025,
