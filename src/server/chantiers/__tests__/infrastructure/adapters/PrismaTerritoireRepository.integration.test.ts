...
describe('PrismaTerritoireRepository', () => {
  ...
  it('doit récupérer la liste des codes de territoire et les codes de leurs territoires enfants', async () => {
    // When
    const territoireCodes = await prismaTerritoireRepository.recupererTerritoireCodesEtTerritoiresCodesEnfantsParTerritoireCode({ territoireCode: 'REG-44' });

    // Then
    expect(territoireCodes).toEqual(['REG-44', 'DEPT-08', 'DEPT-67', 'DEPT-10', 'DEPT-51', 'DEPT-52', 'DEPT-54', 'DEPT-55', 'DEPT-57', 'DEPT-68', 'DEPT-88']);
  });
