...
export class PrismaIndicateurRepository implements IndicateurRepository {
  ...
  async recupererPourExports(chantierId: string, territoireCodesLecture: string[], jalon: number, estAvecCadrage: boolean = false): Promise<IndicateurPourExport[]> {
    ...
    if (estAvecCadrage) {
      ...
      listeMetadataIndicateursComplementaires = await prisma.metadata_indicateurs_complementaire.findMany({
        where: {
          indic_id: {
            in: result.map(indicateur => indicateur.indicateur_identite.id),
          },
    ...
    return result.map(indicateurPourExport => {
      ...
      const informationMetadataIndicateur = estAvecCadrage ? listeMetadataIndicateurs.find(metadataIndicateur => metadataIndicateur.indic_id === indicateurPourExport.indicateur_identite.id) : null;
      const informationMetadataIndicateurComplementaire = estAvecCadrage ? listeMetadataIndicateursComplementaires.find(metadataIndicateurComplementaire => metadataIndicateurComplementaire.indic_id === indicateurPourExport.indicateur_identite.id) : null;

      const informationCadrage = estAvecCadrage ? {
        description: informationMetadataIndicateur?.indic_descr || null,
        methodeCalcul: informationMetadataIndicateur?.indic_methode_calcul || null,
        source: informationMetadataIndicateur?.indic_source || null,
        periodesMiseAJour: informationMetadataIndicateurComplementaire?.periodicite || null,
        delaiDisponibilite: informationMetadataIndicateurComplementaire?.delai_disponibilite || null,
      } : {
        description: null,
        methodeCalcul: null,
        source: null,
