...
describe('RécupérerObjectifsUseCase', () => {
  ...
  beforeEach(() => {
    ...
    récupérerObjectifsUseCase = new RécupérerObjectifsUseCase({ objectifRepository, decisionStrategiqueRepository });
  });
  ...
  it('doit récupérer les objectifs associés à un chantier', async () => {
    ...
    const decisionStrategique1Old = new DecisionStrategiqueBuilder().withType('decision_strategique').withContenu('Suivi des décisions Old').withDate('2022-05-01T00:00:00.000Z').build();
    const decisionStrategique1 = new DecisionStrategiqueBuilder().withType('decision_strategique').withContenu('Suivi des décisions').withDate('2023-05-01T00:00:00.000Z').build();
    ...
    expect(objectifsResult.get('decision_strategique')).toEqual('Suivi des décisions');
  });
});
