...
describe("RécupererChantierFicheConducteurUseCase", () => {
  ...
  beforeEach(() => {
    ...
    récupererChantierFicheConducteurUseCase =
      new RécupererChantierFicheConducteurUseCase({
        chantierRepository,
  ...
  it("doit récupérer le chantier associé au code", async () => {
    ...
    const chantier = new ChantierBuilder()
      ...
      .withNom("Chantier 1")
      .build();
    chantierRepository.récupérerParIdEtParTerritoireCode.mockResolvedValue(
      chantier,
    );
    ...
    // When
    const chantierResult = await récupererChantierFicheConducteurUseCase.run({
      chantierId: id,
      territoireCode,
      jalon,
    });
    // Then
    expect(
      chantierRepository.récupérerParIdEtParTerritoireCode,
  ...
  it("doit récupérer les indicateurs associé au chantier", async () => {
    ...
    const indicateur1 = new IndicateurBuilder()
      .withNom("Indicateur 1")
      .withType("IMPACT")
      .build();
    const indicateur2 = new IndicateurBuilder()
      .withNom("Indicateur 2")
      .withType("IMPACT")
      .build();
    const indicateur3 = new IndicateurBuilder()
    ...
    // Then
    expect(
      indicateurRepository.récupérerIndicImpactParChantierId,
    ).toHaveBeenNthCalledWith(1, "CH-168", jalon);
    expect(chantierResult.indicateurs).toHaveLength(3);
    expect(
      chantierResult.indicateurs.map((indic) => indic.nom),
    ).toIncludeSameMembers(["Indicateur 1", "Indicateur 2", "Indicateur 3"]);
    expect(
      chantierResult.indicateurs.map((indic) => indic.type),
    ).toIncludeSameMembers(["IMPACT", "IMPACT", "Q_SERV"]);
  });
});
