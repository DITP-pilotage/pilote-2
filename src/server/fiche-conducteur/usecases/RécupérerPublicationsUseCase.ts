...
export class RécupérerPublicationsUseCase {
  ...
  async run({
    ...
    Map<ObjectifType | DecisionStrategiqueType | CommentaireType, string>
  > {
    const listeObjectifs = await this.objectifRepository
      .listerObjectifParChantierId({ chantierId })
      .then((listeObjectifsResult) =>
        listeObjectifsResult.sort((objectif1, objectif2) =>
          objectif1.date.localeCompare(objectif2.date),
        ),
      );

    const listeDecisionsStrategique = await this.decisionStrategiqueRepository
      .listerDecisionStrategiqueParChantierId({ chantierId })
      .then((listeDecisionsStrategiqueResult) =>
        listeDecisionsStrategiqueResult.sort(
          (decisionStrategique1, decisionStrategique2) =>
            decisionStrategique1.date.localeCompare(decisionStrategique2.date),
        ),
      );

    const listeCommentaires = await this.commentaireRepository
      .listerCommentaireParChantierId({ chantierId })
      .then((listeCommentairesResult) =>
        listeCommentairesResult.sort((commentaire1, commentaire2) =>
          commentaire1.date.localeCompare(commentaire2.date),
        ),
      );

    return [
      ...listeObjectifs,
      ...listeDecisionsStrategique,
      ...listeCommentaires,
    ].reduce((acc, val) => {
