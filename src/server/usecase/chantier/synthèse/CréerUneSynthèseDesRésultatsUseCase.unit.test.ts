...
describe("CréerUneSynthèseDesRésultatsUseCase", () => {
  test("créé une synthèse des résultats", async () => {
    ...
    //WHEN
    await créerUneSynthèseDesRésultats.run(
      chantierId,
      territoireCode,
      contenu,
      auteur,
    ...
    //THEN
    expect(stubSynthèseDesRésultatsRepository.créer).toHaveBeenNthCalledWith(
      1,
      chantierId,
      territoireCode,
      RANDOM_UUID,
  ...
  test("retourne la synthèse des résultats créée", async () => {
    ...
    //WHEN
    const synthèseDesRésultatsCréée = await créerUneSynthèseDesRésultats.run(
      chantierId,
      territoireCode,
      contenu,
      auteur,
