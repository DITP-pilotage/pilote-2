...
describe("CréerUnObjectifUseCase", () => {
  test("créé un objectif", async () => {
    ...
    const créerUnObjectif = new CréerUnObjectifUseCase({
      objectifRepository: stubObjectifRepository,
    });
    ...
    //THEN
    expect(stubObjectifRepository.créer).toHaveBeenNthCalledWith(
      1,
      chantierId,
      RANDOM_UUID,
      contenu,
  ...
  test("retourne l'objectif créé", async () => {
    ...
    //WHEN
    const objectifCréé = await créerUnObjectif.run(
      chantierId,
      contenu,
      auteur,
      type,
