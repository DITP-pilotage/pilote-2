...
describe("CreerTokenAPIUseCase", () => {
  ...
  beforeEach(() => {
    ...
    creerTokenAPIUseCase = new CreerTokenAPIUseCase({
      tokenAPIService,
  ...
  it("doit creer un token api", async () => {
    ...
    expect(
      tokenAPIInformationRepository.sauvegarderTokenAPIInformation,
    ).toHaveBeenNthCalledWith(1, { email, dateCreation: anyString() });
  ...
  it("quand un token existe déjà, doit supprimer token existant et en créer un nouveau", async () => {
    ...
    const tokenAPIInformation = new TokenAPIInformationBuilder()
      ...
      .build();
    utilisateurRepository.estPresent.mockResolvedValue(true);
    tokenAPIInformationRepository.recupererTokenAPIInformation.mockResolvedValue(
      tokenAPIInformation,
    );
