...
describe("UtilisateurAuthentifieJWTService", () => {
  ...
  beforeEach(() => {
    ...
    utilisateurAuthentifieJWTService = new UtilisateurAuthentifieJWTService({
  ...
  it("quand l'utilisateur est toujours présent dans la base utilisateur, doit retourner l'utilisateur authentifie associe au token jwt", async () => {
    ...
    const habilitationsAPI = new HabilitationAuthentitificationAPIBuilder()
      .ajouterHabilitationLecture("chantiers", [
        "7a33ee55-b74c-4464-892b-b2b7fdc3bc58",
      ])
      .build();
    const utilisateur = new UtilisateurBuilder()
      .withEmail(email)
      .withProfil(ProfilEnum.CABINET_MTFP)
      .withHabilitations(habilitationsAPI)
      .build();
    ...
    tokenAPIRepository.recupererTokenAPIInformation.mockResolvedValue(
      tokenAPIInformation,
    );
    profilRepository.estAutoriseAAccederAuxChantiersBrouillons.mockResolvedValue(
      true,
    );

    // When
    const utilisateurAuthentifie =
      await utilisateurAuthentifieJWTService.recupererUtilisateurAuthentifie(
        tokenJWT,
      );
    profilRepository.estAutoriseAAccederAuxChantiersBrouillons.mockResolvedValue(
      true,
    ...
    // Then
    expect(
      profilRepository.estAutoriseAAccederAuxChantiersBrouillons,
    ).toHaveBeenNthCalledWith(1, { profilCode: ProfilEnum.CABINET_MTFP });
    ...
    expect(utilisateurAuthentifie.profilAAccèsAuxChantiersBrouillons).toEqual(
      true,
    );
  });
