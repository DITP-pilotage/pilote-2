...
export class UtilisateurAuthentifieJWTService {
  ...
  constructor({ utilisateurRepository, tokenAPIRepository, profilRepository }: {
  ...
  async recupererUtilisateurAuthentifie(tokenJWT: string): Promise<UtilisateurAuthentifie> {
    const decodedToken = await new TokenAPIJWTService({ secret: configuration.tokenAPI.secret }).decoderTokenAPI(tokenJWT);
    const email = decodedToken?.email!;
    const utilisateur = await this.utilisateurRepository.récupérer(email);

    const utilisateurDuToken = await this.tokenAPIRepository.recupererTokenAPIInformation({ email });

    if (!utilisateur || !utilisateurDuToken) {
      throw new ForbiddenError('L\'utilisateur n\'existe pas ou plus dans la base de données utilisateur de pilote');
    }
