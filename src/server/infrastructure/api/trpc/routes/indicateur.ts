...
export const indicateurRouter = créerRouteurTRPC({
  récupererDétailsIndicateurs: procédureProtégée
    ...
    .query(async ({ input, ctx }) => {
      return new RécupérerDétailsIndicateursUseCase(dependencies.getIndicateurRepository())
        .run(input.chantierId, input.territoireCodes, ctx.session.habilitations);
    }),
  ...
  récupererDétailsIndicateur: procédureProtégée
    ...
    .query(async ({ input, ctx }) => {
      ...
      return récupérerDétailIndicateurUseCase.run(input.indicateurId, ctx.session.habilitations, ctx.session.profil);
    }),
});
