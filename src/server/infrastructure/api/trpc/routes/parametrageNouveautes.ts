...
export const parametrageNouveautesRouter = créerRouteurTRPC({
  creer: procédureProtégée.input(z.object({
    contenu: z.string(),
    version: z.string(),
    date: z.string(),
  })).mutation(async ({ input, ctx }) => {
    const estAutoriseAModifierLesNouveautés = ctx.session.profil === ProfilEnum.DITP_ADMIN;
  ...
  modifier: procédureProtégée.input(z.object({
    id: z.string(),
    contenu: z.string(),
    version: z.string(),
    date: z.string(),
    ...
    return getContainer('parametrageNouveautes').resolve('modifierNouveauteUseCase').execute({ id: input.id, contenu: input.contenu, version: input.version, date: input.date });
  }),
});
