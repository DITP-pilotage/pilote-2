...
export async function handleExportMetadataIndicateurs(
  ...
  const listeMetadataIndicateur = await getContainer("parametrageIndicateur")
    .resolve("récupérerListeMetadataIndicateurUseCase")
    .run(
      (request.query?.chantierIds as string[]) || [],
      (request.query?.perimetreIds as string[]) || [],
      request.query?.estTerritorialise === "true",
      request.query?.estBarometre === "true",
    );
  ...
  listeMetadataIndicateur.forEach((metadataIndicateur) => {
    stringifier.write([
      ...
      metadataIndicateur.zgApplicable,
      ...
      metadataIndicateur.poidsPourcentEvalNat,
      metadataIndicateur.poidsPourcentEvalReg,
      metadataIndicateur.poidsPourcentEvalDept,
