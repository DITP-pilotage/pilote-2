...
import {CODES_MAILLES, NOMS_MAILLES} from '@/server/infrastructure/maille/mailleSQLParser';
...
export default class IndicateurSQLRepository implements IndicateurRepository {
  ...
  async getEvolutionIndicateur(chantierId: string, indicateurId: string, maille: Maille, codesInsee: string[]): Promise<EvolutionIndicateur[]> {
    const indicateurs: indicateur[] = await this.prisma.indicateur.findMany({
      where: {
    ...
    for (const item of indicateurs) {
      result.push({
        ...
        évolutionDateValeurActuelle: item.evolution_date_valeur_actuelle.map(dateToDateStringWithoutTime),
      });
    }
