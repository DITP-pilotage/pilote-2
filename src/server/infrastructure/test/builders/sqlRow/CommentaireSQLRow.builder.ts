...
export default class CommentaireRowBuilder {
  ...
  constructor() {
    ...
    if (
      this._type === "freins_a_lever" ||
      this._type === "actions_a_venir" ||
      this._type === "actions_a_valoriser" ||
      this._type === "autres_resultats_obtenus_non_correles_aux_indicateurs"
    ) {
      this._maille = "NAT";
      this._codeInsee = "FR";
    } else if (
      this._type === "commentaires_sur_les_donnees" ||
      this._type === "autres_resultats_obtenus"
    ) {
      this._maille = faker.helpers.arrayElement(["DEPT", "REG"]);
      this._codeInsee = faker.helpers.arrayElement(
        retourneUneListeDeCodeInseeCohérentePourUneMaille(this._maille),
      );
    } else {
    ...
    this._type =
      maille === "NAT"
        ? CODES_TYPES_COMMENTAIRES[
            faker.helpers.arrayElement(typesCommentaireMailleNationale)
          ]
        : CODES_TYPES_COMMENTAIRES[
