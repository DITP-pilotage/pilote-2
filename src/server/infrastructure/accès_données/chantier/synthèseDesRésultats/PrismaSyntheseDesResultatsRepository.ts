import { synthese_des_resultats, utilisateur as UtilisateurPrisma, PrismaClient } from '@prisma/client';
import { SyntheseDesResultatsRepository } from '@/server/domain/chantier/synthèseDesRésultats/SynthèseDesRésultatsRepository.interface';
...
import { SyntheseDesResultats } from '@/server/domain/chantier/synthèseDesRésultats/SynthèseDesRésultats.interface';
...
export class PrismaSyntheseDesResultatsRepository implements SyntheseDesResultatsRepository {
  ...
  private mapperVersDomaine(synthèse: synthese_des_resultats & { auteur_synthese: UtilisateurPrisma | null } | null): SyntheseDesResultats {
  ...
  async créer(chantierId: string, territoireCode: string, id: string, contenu: string, auteur_id: string, météo: Météo, date: Date): Promise<SyntheseDesResultats> {
  ...
  async récupérerLaPlusRécente(chantierId: string, territoireCode: string): Promise<SyntheseDesResultats> {
    const { maille, codeInsee } = territoireCodeVersMailleCodeInsee(territoireCode);
  ...
  async récupérerHistorique(chantierId: string, territoireCode: string): Promise<SyntheseDesResultats[]> {
    const { maille, codeInsee } = territoireCodeVersMailleCodeInsee(territoireCode);
  ...
  async récupérerLesPlusRécentesGroupéesParChantier(chantiersIds: Chantier['id'][], maille: Maille, codeInsee: CodeInsee): Promise<Record<Chantier['id'], SyntheseDesResultats>> {
