...
describe("DécisionStratégiqueSQLRepository", () => {
  ...
  describe("#récupérerLePlusRécent", () => {
    it("Retourne la décision stratégique la plus récente pour un chantier avec son contenu, auteur et date", async () => {
      ...
      // When
      const résultat =
        await décisionStratégiqueRepository.récupérerLaPlusRécente(chantierId);
    ...
    it("Lorsque l'auteur id est null, retourne Auteur Inconnu", async () => {
      ...
      // When
      const résultat =
        await décisionStratégiqueRepository.récupérerLaPlusRécente("CH-003");
    ...
    it("Lorsque l'auteur id est non null, retourne prenom + nom de l'utilisateur associé", async () => {
      ...
      // When
      const résultat =
        await décisionStratégiqueRepository.récupérerLaPlusRécente("CH-004");
  ...
  describe("RécupérerLHistorique", () => {
    it("Retourne toutes les publications de décisions stratégiques pour un chantier, dans l'ordre décroissant", async () => {
      ...
      // When
      const résultat =
        await décisionStratégiqueRepository.récupérerHistorique(chantierId);

      // Then
      expect(résultat[0]?.date).toStrictEqual(
        new Date("2023-03-30").toISOString(),
      );
      expect(résultat[1]?.date).toStrictEqual(
        new Date("2023-01-30").toISOString(),
      );
      expect(résultat[2]?.date).toStrictEqual(
  ...
  describe("créer", () => {
    ...
    it("Crée la décision stratégique en base", async () => {
      ...
      // When
      await décisionStratégiqueRepository.créer(
        chantierId,
        id,
        contenu,
        type,
      ...
      // Then
      const décisionStratégiqueCrééeEnBase =
        await prisma.decision_strategique.findUnique({ where: { id: id } });
    ...
    it("Retourne la décision stratégique créée", async () => {
      ...
      // When
      const décisionStratégiqueCrééeRetournéeParLeRepo =
        await décisionStratégiqueRepository.créer(
          chantierId,
          id,
          contenu,
