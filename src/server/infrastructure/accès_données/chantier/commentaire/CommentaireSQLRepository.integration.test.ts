...
describe("CommentaireSQLRepository", () => {
  ...
  describe("#récupérerLePlusRécent", () => {
    it("Retourne null quand pas de commentaire de ce type en base pour ce chantier", async () => {
      // When
      const résultat = await prismaCommentaireRepository.récupérerLePlusRécent(
        "CH-001",
        "NAT-FR",
    ...
    it("Retourne le commentaire le plus récent pour un type et un chantier", async () => {
      ...
      const résultat = await prismaCommentaireRepository.récupérerLePlusRécent(
        chantierId,
        `${CODES_MAILLES[maille]}-${codeInsee}`,
        "risquesEtFreinsÀLever",
      );
  ...
  describe("#récupérerHistorique", () => {
    test("Retourne, par ordre antéchronologique, tous les commentaires du type donné pour un chantier et un territoire", async () => {
      ...
      // When
      const result = await prismaCommentaireRepository.récupérerHistorique(
        chantierId,
        `${CODES_MAILLES[maille]}-${codeInsee}`,
        typeCommentaire,
      );
  ...
  describe("#créer", () => {
    test("Crée le commentaire en base", async () => {
      ...
      // When
      await prismaCommentaireRepository.créer(
        chantierId,
        "REG-01",
        id,
        contenu,
      ...
      // Then
      const commentaireCrééeEnBase = await prisma.commentaire.findUnique({
        where: { id: id },
      });
    ...
    test("Retourne le commentaire créé", async () => {
      ...
      // When
      const commentaireCréée = await prismaCommentaireRepository.créer(
        chantierId,
        "REG-01",
        id,
        contenu,
