  ...
  territoires.forEach((territoire) => {
    ...
    donnéesTerritoires[territoire.code] = {
      ...
      dateValeurCibleAnnuelle:
        indicateurTerritoireJalon?.date_valeur_cible?.toLocaleString() ?? null,
...
export function parseDétailsIndicateur(
  ...
  const indicateurMailleNationale = prismaIndicateurTerritoire.filter(
    (indicateur) => indicateur.maille === "NAT",
  );
  const indicateurMailleDépartementale = prismaIndicateurTerritoire.filter(
    (indicateur) => indicateur.maille === "DEPT",
  );
  const indicateurMailleRégionale = prismaIndicateurTerritoire.filter(
    (indicateur) => indicateur.maille === "REG",
  );

  return {
    nationale: créerDonnéesTerritoires(
      territoires.filter((territoire) => territoire.maille === "NAT"),
      indicateurMailleNationale,
    ),
    departementale: créerDonnéesTerritoires(
      territoires.filter((territoire) => territoire.maille === "DEPT"),
      indicateurMailleDépartementale,
    ),
    regionale: créerDonnéesTerritoires(
      territoires.filter((territoire) => territoire.maille === "REG"),
      indicateurMailleRégionale,
    ),
  };
}
