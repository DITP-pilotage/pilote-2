...
describe('HistorisationModificationSQLRepository', () => {
  ...
  describe('#anonymiserAuteurs', () => {
    test('doit anonymiser l\'auteur des historisations de l\'utilisateur supprimé', async () => {
      ...
      // When
      await historisationModificationSQLRepository.anonymiserAuteurs([auteurId2], 'auteur.inconnu@modernisation.gouv.fr');

      // Then
      const historisationAvecAuteurAnonyme = await prisma.historisation_modification.findMany({ where: { id_auteur: auteurId3 } });
      expect(historisationAvecAuteurAnonyme).toHaveLength(2);
      expect(historisationAvecAuteurAnonyme[0].id).toStrictEqual('b699907e-43c4-43be-8d8d-185fca1b2e50');
      expect(historisationAvecAuteurAnonyme[1].id).toStrictEqual('e3885e40-caab-4fb6-acf4-0c8f66c9e290');
    });
  });
