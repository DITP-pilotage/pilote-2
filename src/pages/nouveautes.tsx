...
export const getServerSideProps: GetServerSideProps<{ estAutoriseAModifierLesNouveautés: boolean }> = async ({ req, res }) => {
  ...
  const listeNouveautes = await getContainer('parametrageNouveautes').resolve('listerNouveautesUseCase').execute();

  const date = new Date();
  date.setFullYear(date.getFullYear() + 1);
  res.setHeader('Set-Cookie', `derniereVersionNouveauteConsulte=${listeNouveautes?.[0]?.version || '1.0.0'}; path=/; samesite=lax; expires="${date.toUTCString()}";`);
