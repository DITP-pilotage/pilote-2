...
function récupérerDétailIndicateursPourUnChantierDunTerritoireSelectionné(
  chantierId: Chantier["id"],
  codeInsee: string,
  mailleAssociéeAuTerritoireSélectionné: string,
) {
  return fetch(
    `/api/chantier/${chantierId}/indicateurs?codesInsee=${codeInsee}&maille=${mailleAssociéeAuTerritoireSélectionné}`,
  );
}

function creerListeCodeInsee(territoiresComparés: DétailTerritoire[]) {
  return territoiresComparés
    .map(({ codeInsee }) => `codesInsee=${codeInsee}`)
    .join("&");
}
...
export default function useImportIndicateur(chantierId: Chantier["id"]) {
  ...
  const [détailsIndicateurs, setDétailsIndicateurs] =
    useState<DétailsIndicateurs | null>(null);

  useEffect(() => {
    if (
      estUnUniqueTerritoireComparé(territoiresComparés) &&
      estUnTerritoireCodeInseeFR(territoiresComparés)
    ) {
      return;
    }

    const { codesInsee, maille } =
      territoiresComparés.length > 0
        ? {
    ...
    récupérerDétailIndicateursPourUnChantierDunTerritoireSelectionné(
      chantierId,
      codesInsee,
      maille,
    )
